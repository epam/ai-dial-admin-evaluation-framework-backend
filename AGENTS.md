...
- **`test_cases` query entity + suite `testCaseFilter`** — `test_cases` is a **complex** queryable entity keyed by `dataset_id` (`TestCasesSchemaProvider`); its flattened `data::<field>` typing is **dataset-specific**, so its `PostgresTestCaseEntityResolver.bindings(query)` is **instance-aware**: it requires a top-level `dataset_id` equality filter (missing/non-UUID → 400) and builds typed bindings via `TestCaseFieldBindingsBuilder` — this is simply that entity's own implementation of the same `bindings(StructuredQuery)` method every resolver implements, not a separate bypass-the-cache code path (cache-backed `test_suites`/`eval_summaries`/`metric_score_results` just ignore the query and return a precomputed map). `co`/`nc` on an `ARRAY`-typed `data::<field>` translate to JSONB containment (§ Query DSL function catalog). A suite carries an optional `testCaseFilter` (JSONB, mirrors `overallScore`) **validated at write time** (`TestSuiteService` → 400 on unknown field / unbound suite). It is applied at run time — combined `is_valid AND NOT excluded AND filter` — via the inverted **`service.domain.job.RunnableTestCaseSelector`** interface (impl `QueryDslRunnableTestCaseSelector` in `experimental.query.service`, same inversion as `MetricScoreComputation`); `RunnableTestCaseCounter` (guard #4) and `TestSuiteEvaluationJob.attemptSnapshot` call it, so there is **no** `service → experimental` edge. For a **multi-turn** case the filter is **ALL-turns-match** (universal quantifier): `QueryDslRunnableTestCaseSelector.compile()` re-points `data::<field>` bindings to a per-turn element `elem` (via the `TestCaseFieldBindingsBuilder.build(datasetId, dataSource)` overload), compiles the filter once against `elem`, and wraps it as `NOT EXISTS(SELECT 1 FROM jsonb_array_elements(coalesce(multi_turn_data, jsonb_build_array(data))) AS t(elem) WHERE (<filter@elem>) IS NOT TRUE)` — a turn whose predicate is false *or unknown* (missing field) fails. Added **only when a filter is present** (no-filter selection is byte-identical to before); single-turn is the trivial 1-element array. `FilterTranslator` is unchanged.

- **Multi-turn — array-based, one result row per turn** — multi-turn is **emergent from data, not a suite flag**: a multi-turn case is a **single `test_cases` row** carrying an ordered `multi_turn_data JSONB` array of per-turn data maps (V1.27). It is **mutually exclusive** with single-turn `data` (multi-turn ⇒ `data = '{}'`; DB CHECK `chk_test_cases_multi_turn_exclusive`); the discriminator is `multi_turn_data != null`. Turns are inherently contiguous `0..N-1` — there is **no** `multiTurnId`, `last_turn_index`, `broken` flag, or `MultiTurnAssembler` (all POC constructs, dropped). Authoring: `multiTurnData` on request/response/batch DTOs (`@JsonInclude(NON_NULL)`), PATCH-able with mutual-exclusivity clearing; `MultiTurnFieldsValidator` enforces mutual exclusivity + non-empty array (→ 400); `TestCaseValidationService.validateMultiTurn` validates each turn against the dataset schema (case `is_valid` iff every turn passes; warnings carry a `turnIndex`) and invalidates an over-cap case (configurable `test-case.multi-turn.max-turns`, default 10) with a warning (not 400). CSV import/export stays **flat**: export multiplies a case to one row per turn (reserved `turnIndex` column after `testCaseName`, blank for single-turn); import assembles a **contiguous run** of same-`testCaseName` rows into one `multiTurnData` (sorted by `turnIndex`; non-contiguous name / duplicate `turnIndex` → conflict warning). **Snapshot** freezes `multi_turn_data` into `test_case_run_inputs` (one input row per case, V1.28). **Execution:** `EvaluationWorker.execute` returns `List<TestCaseRunResult>` and dispatches on `input.multiTurnData != null` to `MultiTurnExecutor` (`service.domain.job`), which drives a sequential chat-completions turn loop under one permit — accumulate `messages` history, resolve the suite's single template/bindings against each turn's own map, re-send full history non-streaming, append `choices[0].message` verbatim, extract per-turn columns, persist one `TestCaseRunResult` per turn (`turn_index`/`total_turns`, shared `test_case_id`/`trace_id`); fail-fast (first failing turn → one ERROR row, later turns unsent). `DeploymentTurnInvoker`/`TurnOutcome`/`DeploymentInvocationSupport` own the per-turn non-streaming call + retry/backoff; `QuietJsonService` is the failure-tolerant JSON facade. `ResultBatchWriter.addResults(buffer, list)` advances progress by **one unit per conversation**. Analytics `test_case_run_results`/`test_case_eval_summaries` gain `turn_index`/`total_turns` (V1.13/V1.14, natural keys extended). **Guard:** an `MCP_TOOL` suite bound to a dataset with any multi-turn case is rejected at run creation (409 `INVALID_OPERATION`) via `TestCaseService.datasetHasMultiTurnCases` (→ `TestCaseRepository.existsMultiTurnByDatasetId`).
