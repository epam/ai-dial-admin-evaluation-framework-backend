...
- **`test_cases` query entity + suite `testCaseFilter`** — `test_cases` is a **complex** queryable entity keyed by `dataset_id` (`TestCasesSchemaProvider`); its flattened `data::<field>` typing is **dataset-specific**, so its `PostgresTestCaseEntityResolver.bindings(query)` is **instance-aware**: it requires a top-level `dataset_id` equality filter (missing/non-UUID → 400) and builds typed bindings via `TestCaseFieldBindingsBuilder` — this is simply that entity's own implementation of the same `bindings(StructuredQuery)` method every resolver implements, not a separate bypass-the-cache code path (cache-backed `test_suites`/`eval_summaries`/`metric_score_results` just ignore the query and return a precomputed map). `co`/`nc` on an `ARRAY`-typed `data::<field>` translate to JSONB containment (§ Query DSL function catalog). A suite carries an optional `testCaseFilter` (JSONB, mirrors `overallScore`) **validated at write time** (`TestSuiteService` → 400 on unknown field / unbound suite). It is applied at run time — combined `is_valid AND NOT excluded AND filter` — via the inverted **`service.domain.job.RunnableTestCaseSelector`** interface (impl `QueryDslRunnableTestCaseSelector` in `experimental.query.service`, same inversion as `MetricScoreComputation`); `RunnableTestCaseCounter` (guard #4) and `TestSuiteEvaluationJob.attemptSnapshot` call it, so there is **no** `service → experimental` edge. For a **multi-turn** case the filter is **scope-aware ALL-turns-match**: `QueryDslRunnableTestCaseSelector.compile()` builds **scope-aware bindings** via `TestCaseFieldBindingsBuilder.buildScoped(datasetId, elem)` — **per-turn** (`perTurn=true`) fields bind to the per-turn element `elem`, **shared** (`perTurn` false/absent) fields bind to the outer row's `TEST_CASES.DATA` (row-level, constant across turns) — compiles the filter once, and wraps the per-turn portion as `NOT EXISTS(SELECT 1 FROM jsonb_array_elements(coalesce(multi_turn_data, jsonb_build_array(data))) AS t(elem) WHERE (<filter@elem>) IS NOT TRUE)` (correlated to the outer row so shared references resolve inside the lateral) — a turn whose per-turn predicate is false *or unknown* (missing field) fails. Added **only when a filter is present** (no-filter selection is byte-identical to before); single-turn is the trivial 1-element array. `FilterTranslator` is unchanged.

- **Multi-turn — array-based, one result row per turn; shared + per-turn data** — multi-turn is **emergent from data, not a suite flag**: a multi-turn case is a **single `test_cases` row** carrying an ordered `multi_turn_data JSONB` array of per-turn data maps (V1.27); the discriminator is `multi_turn_data != null`. `data` and `multi_turn_data` **coexist** (no mutual exclusivity, no DB CHECK): `data` holds the case's **shared** (test-case-level, constant across turns) fields and each turn map holds the **per-turn** fields. Field **scope** is declared per field on the dataset schema via `FieldDefinitionDto.perTurn` (nullable `Boolean`, absent/false ⇒ shared — so pre-existing schemas and all single-turn cases are unchanged; single-turn always keeps every field in `data` regardless of scope). `TestCaseFieldScopeResolver` (`service.domain`) is the single source that splits a schema/flat map into shared vs per-turn. Turns are inherently contiguous `0..N-1` — **no** `multiTurnId`, `last_turn_index`, `broken` flag, or `MultiTurnAssembler`. Authoring: `multiTurnData` on request/response/batch DTOs (`@JsonInclude(NON_NULL)`); `data` and `multiTurnData` are **independently PATCH-able** (patching one no longer clears the other; `multiTurnData:null` reverts to single-turn). `MultiTurnFieldsValidator` enforces non-empty array + **scope placement** (a per-turn field in `data`, or a shared field in a turn map → **400**); `TestCaseValidationService.validateMultiTurn(sharedData, turns, schema, …)` validates shared fields against `data` and per-turn fields against every turn (case `is_valid` iff no shared-field warning and every turn passes; per-turn warnings carry a `turnIndex`; all-empty turn maps are valid when no required per-turn field), and invalidates an over-cap case (configurable `test-case.multi-turn.max-turns`, default 10) with a warning (not 400). CSV import/export stays **flat**: export merges shared `data` into every turn row so **shared columns repeat** on each row; import routes each column by its dataset-schema scope (`CsvImportService.applyScopeFromDataset` re-applies persisted `perTurn` onto the OVERRIDE-rebuilt validation schema), requires a case's shared columns identical across rows (mismatch → conflict warning), and assembles a **contiguous run** of same-`testCaseName` rows into `data` (shared) + `multiTurnData` (per-turn, sorted by `turnIndex`; non-contiguous name / duplicate `turnIndex` → conflict warning). **Snapshot** freezes both `data` (shared) and `multi_turn_data` into `test_case_run_inputs` (one input row per case, V1.28). **Execution:** `EvaluationWorker.execute` returns `List<TestCaseRunResult>` and dispatches on `input.multiTurnData != null` to `MultiTurnExecutor` (`service.domain.job`), which drives a sequential chat-completions turn loop under one permit — each turn resolves the suite's single template/bindings against its **merged effective view** `merge(shared data, turn[i])` (per-turn keys win), accumulates `messages` history, re-sends full history non-streaming, appends `choices[0].message` verbatim, extracts per-turn columns, and persists one `TestCaseRunResult` per turn whose `testCaseData` is that merged view (so the conditional-metric `data` namespace and metric input see shared fields with **no** `ConditionExpressionEvaluator` change); fail-fast (first failing turn → one ERROR row, later turns unsent). `DeploymentTurnInvoker`/`TurnOutcome`/`DeploymentInvocationSupport` own the per-turn non-streaming call + retry/backoff; `QuietJsonService` is the failure-tolerant JSON facade. `ResultBatchWriter.addResults(buffer, list)` advances progress by **one unit per conversation**. Analytics `test_case_run_results`/`test_case_eval_summaries` gain `turn_index`/`total_turns` (V1.13/V1.14, natural keys extended). **Guard:** an `MCP_TOOL` suite bound to a dataset with any multi-turn case is rejected at run creation (409 `INVALID_OPERATION`) via `TestCaseService.datasetHasMultiTurnCases` (→ `TestCaseRepository.existsMultiTurnByDatasetId`).

- **Conditional metric execution (`condition` on TSMD)** — a TSMD carries an optional `condition VARCHAR(2000)` (V1.26, nullable ⇒ always run) deciding per **result row (per turn)** whether the metric runs. `ConditionExpressionEvaluator` (`service.domain`) evaluates it as **JSONata** (`JsonataEvaluationService`) over a namespaced dictionary `{data, response, turn:{index,total,last}}` — `turn.last = index == total - 1` (contiguous turns); the dictionary is an `ObjectNode` serialized **preserving explicit nulls** (never the shared `NON_NULL` mapper). **Write time** (`TestSuiteMetricDefinitionService.create/update`): `validate(condition)` → hard 400 on invalid JSONata. **Run time** (`InProcessMetricEvaluationExecutor`, synchronously before async dispatch, only on SUCCESS rows): clean `true` → run; clean `false` → omit the metric entirely; throws/non-boolean/null → `TsmdEvaluationResult.ConditionError` → no `metricValues` entry + a wholesale `metricInfos[tsmd] = {error}` (the `metricError::<name>` export column), and the row stays SUCCESS (`checkForErrors` ignores `ConditionError`). Only dispatched metrics are reconciled for timeout/failure.
