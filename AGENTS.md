...
- **Query DSL subquery-valued `in`** — the `in` predicate's right operand may be an `array` of literals **or** a `subquery` (`SubqueryExpr`, wire `{"type":"subquery","query":{…}}`), letting one `POST /api/v1/queries/execute` express "members of the top-N groups" (e.g. all metric scores for a suite's latest N runs) as **one SQL statement**. `FilterTranslator` compiles it to a nested `SELECT` — builds the nested query via a **lazy `ObjectProvider<StructuredQueryBuilder>`** (breaks the builder↔translator cycle) and wraps it in a derived table whose **first select column is the membership key** (`left IN (SELECT firstCol FROM (<subquery>) …)`), so the subquery may also select an aggregate purely to drive its own `ORDER BY`/`LIMIT` (e.g. `max(computed_at_ms)`). Constraints (all → 400): **same entity** as the enclosing query (reuses its table + bindings), and `subquery` valid **only** as an `in` right operand (`ExprTranslator` keeps a defensive reject). The `dsl`/`table`/`entity` needed for compilation live in a `TranslationContext` held for the call in a **`ThreadLocal`** on `FilterTranslator` (mirroring `configuration.security.AuthorizationTokenHolder`), set by the 3-arg `toCondition(node, bindings, ctx)` overload with save/restore in `finally` (so sibling/nested subqueries see the right context and nothing leaks) and read only by `subquerySelect` — the recursive methods keep their plain `(node, bindings)` signatures. The public 2-arg `toCondition(node, bindings)` stays for non-DSL callers (`QueryDslRunnableTestCaseSelector`): no context is set, so a subquery operand is rejected. `QueryParameterResolver` recurses into `SubqueryExpr`.
