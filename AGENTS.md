...
- All Spring component classes MUST have `@LogExecution` annotation at the class level (`@RestController`, `@Controller`, `@Service`, `@Repository`, `@Component`, `@Configuration`). Import: `com.epam.aidial.evaluation.runner.config.logging.LogExecution`. **Exception 1:** a component whose public method takes a plaintext secret (API key, raw token, password) as a `String` MUST NOT be `@LogExecution` — the opt-in trace advisor logs arguments verbatim. Wrap the secret in a redacting type (e.g. `CallerCredential`) or leave the class unannotated and add it to `SecretHandlingLoggingTest`. **Exception 2:** a `@Primary` wrapper that exists only to delegate to an already-annotated bean omits it, because annotating both logs every call twice — see `JooqStructuredQueryExecutorExtender` in [query-result-page-extension](docs/patterns/query-result-page-extension.md). No test enforces exception 2 either way, so state the reason in the class javadoc when you take it.
...
| [Query result page extension](docs/patterns/query-result-page-extension.md) | `QueryResultPageExtender` SPI + `@Primary` coordinator; open key set; per-extender failure isolation; an extender bounding an external lookup with a timed `Future.get` self-degrades (logs once, returns the received page) |
...
| [`test_suite_runs` query entity](docs/patterns/test-suite-runs-query-entity.md) | Fully derived table excludes heavy JSONB cols (one `run_config` scalar, `number_of_runs`, still published); `metric_names` scalar subquery pulled up; snapshot-only refs; `computed_at_ms`/`computation_id` tiebreak |
