...
- **Multi-turn conversation (`multiTurn`) — one result row per turn** — when a DEPLOYMENT suite has `multiTurn == true`, it uses its **single regular `inputBindings`** (no separate per-turn binding config). Per-turn variation comes from the **data**: the test-case columns bound by those bindings hold **arrays**, and `MultiTurnConversationExecutor` (delegated from `EvaluationWorker` on `context.isSnapshotMultiTurn()`) iterates them — turn `i` resolves the template with element `i` of each array-valued bound column (scalar columns and `constantValue` bindings broadcast unchanged), computed by `ConversationTurnPlanner` (`turnPlan.project(data, i)`). **Turn count is a per-test-case concern**: `N` = the common length of the array-valued bound columns *in that test case's data*, so two test cases in the same suite can run different turn counts (capped at `MAX_CONVERSATION_TURNS`). Each turn re-sends the full accumulated `messages` history (turn invocation delegated to `DeploymentTurnInvoker` → `TurnOutcome`); the assistant reply appended after each turn is the **full `choices[0].message` object, appended verbatim** (preserving `tool_calls`/`refusal`/structured content/etc.), never a reconstructed `{role, content}` — a turn aborts only when the 2xx response carries **no `choices[0].message` object** (a message with no string `content`, e.g. a tool-call turn, is valid). Because the response body is serialized with the shared `NON_NULL` mapper before the message is read, a reply's literal `content: null` is appended as absent `content`. Config-time validation (`SuiteValidationService.validateMultiTurnBody`) for `multiTurn` is just the normal `bindingValidator` run plus a chat-completions body check (top-level `messages` array); array-shape/length is a runtime-only concern.
  **Per-turn emission (the core of this design):** `EvaluationWorker.execute(...)` returns a **`List<TestCaseRunResult>`** — single-turn/MCP paths return a one-element list (`turn_index=0, total_turns=1`); the multi-turn executor emits **one scalar `TestCaseRunResult` per turn**, each with `turn_index=i`, `total_turns=N`, `test_case_data`=that turn's projected (scalar) data, `request_body`=the full accumulated request, `response_body`=that turn's raw response body, and `extracted_columns`=a **plain object of scalars** (the *same* single-turn shape — there is **no** column-major array / JSONata / `ArrayBindingTypeMismatchDetector` machinery; that was all rolled back). Each row's `extraction_warnings` are that turn's own warnings (no `stepIndex` stamping — `ExtractionWarningDto` has no `stepIndex`). **Abort semantics:** a mid-conversation turn failure persists `k` `SUCCESS` rows + 1 `ERROR` row (all with `total_turns=N`); a **data-shape** failure (no array-valued bound column, mismatched lengths, over-cap) fails only that test case with a single `ERROR` row carrying the sentinel `turn_index=0, total_turns=0` and returns without throwing — other test cases proceed. `ResultBatchWriter.addResults(buffer, List<TestCaseRunResult>)` buffers all of a conversation's rows and advances the completed-**conversation** progress counter by exactly **one** per call (`notifyProgress(conversationsCompleted, totalCases)`), flushing by row batch size. Metric evaluation reverted to the simple single-value path: `BindingResolver` resolves each binding to the **raw scalar** column value (missing column → fail-fast); `EvalSummary.extractedColumns` stores that turn's scalar object verbatim. `multiTurn` is an additive `SuiteSnapshotDto` field (version stays `"2"`).
  **Turn columns:** analytics tables `test_case_run_results` and `test_case_eval_summaries` carry `turn_index INTEGER NOT NULL DEFAULT 0` and `total_turns INTEGER NOT NULL DEFAULT 1`; their natural-key unique indexes include `turn_index`. Both fields are exposed on `TestCaseRunResultResponseDto`/`EvalSummaryResponseDto` (auto-mapped, same-named `int`) and as CSV export identity columns immediately after `runIndex`. `EvalSummaryBatchWriteItemDto`'s two turn fields are **nullable `Integer`** (defaulting to `0`/`1` in the mapper) so external single-turn callers of `POST /api/v1/analytics/eval-summaries` stay byte-compatible. **Within-conversation turn ordering is client-side**: the keyset `(created_at, id)` cursor spine is unchanged; the frontend groups by `(testCaseId, runIndex)` and sorts each group by `turnIndex`.
- **Conditional metric execution (`condition` on TSMD)** — a TSMD carries an optional `condition` string (`test_suite_metric_definitions.condition VARCHAR(2000)`, nullable ⇒ always run) deciding whether that metric runs **per test-case result** — with per-turn rows this is now **per turn** (each turn is its own result). `ConditionContext` carries `turnIndex`/`totalTurns` (populated in `InProcessMetricEvaluationExecutor` from the result row; single-turn `0/1`), so a future `isLastTurn()` `ConditionFunction` can read them without signature churn — the function itself is out of scope here, only context population lands. `ConditionExpressionEvaluator` (`service.domain`, single entry point) evaluates it against a **namespaced dictionary** `{ "data": <test-case columns>, "response": <extracted columns> }` — same `data`/`response` tokens as the CSV export, built as an `ObjectNode` and serialized **preserving explicit nulls** (never the shared `NON_NULL` mapper, which would drop a present-null column and make it look absent). Detection: a whole trimmed **bare `name()`** (regex `^[A-Za-z_][A-Za-z0-9_]*\(\)$`) is a custom/system function resolved via `ConditionFunctionRegistry` (a `ConditionFunction` SPI mirroring `QueryFunctionRegistry`, **shipped empty** — `isLastTurn()` etc. are future additions now that `ConditionContext` carries turn position); anything else is JSONata via `JsonataEvaluationService` (JSONata functions are `$`-prefixed, so bare `name()` never collides). Custom functions and `evaluate(condition, ConditionContext)` take an **extensible `ConditionContext`** carrier (builder-backed) so future per-turn fields add without signature churn. **Write time** (`TestSuiteMetricDefinitionService.create/update`): `validate(condition)` → **hard 400** on invalid JSONata syntax or an unregistered `name()`; the condition does **not** feed the soft `is_valid` flag and is not persisted as a warning. **Run time** (`InProcessMetricEvaluationExecutor.evaluateAndBuild`, synchronously on the orchestrating thread before any async dispatch): clean boolean `true` → run; clean boolean `false` → **omit the metric entirely** (no `metricValues`/`metricInfos` entry — "absent = intentionally skipped"); any other outcome (throws / non-boolean / null) → new `TsmdEvaluationResult.ConditionError` → **no** `metricValues` entry + a **wholesale** metric-level `metricInfos[tsmd] = {error}` (renders as the `metricError::<name>` export column, relying on `error` not being an output-schema field name). `checkForErrors` **ignores** `ConditionError`, so a bad condition keeps the test-case result `SUCCESS` (never inflates failure counts). Phase-3 aggregate exclusion of a skipped metric is emergent (absent key → JSONB NULL → NULL-skipping SQL aggregates), not a code change.
