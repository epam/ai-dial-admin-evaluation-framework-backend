...
- **Query DSL subquery-valued `in`** — the `in` predicate's right operand may be an `array` of literals **or** a `subquery` (`SubqueryExpr`, wire `{"type":"subquery","query":{…}}`), letting one `POST /api/v1/queries/execute` express "members of the top-N groups" (e.g. all metric scores for a suite's latest N runs) as **one SQL statement**. `FilterTranslator` compiles it (same-entity check, derived-table wrap, first-select-column-is-the-membership-key extraction) via `left IN (SELECT firstCol FROM (<subquery>) …)`, so the subquery may also select an aggregate purely to drive its own `ORDER BY`/`LIMIT` (e.g. `max(computed_at_ms)`). It never depends on `StructuredQueryBuilder` as a bean: the caller passes a `SubqueryContext(dsl, entity, QueryCompiler)` — a plain value carrying the enclosing entity and a `QueryCompiler` **function** (`StructuredQuery -> SelectQuery<Record>`, a `@FunctionalInterface`, never a Spring bean) — as an ordinary parameter to the 3-arg `toCondition(node, bindings, subqueryContext)` overload, threaded through the existing recursive `(node, bindings)` methods exactly like `QueryParameterResolver` threads its own state. `StructuredQueryBuilder.build`/`countRows`/`buildAggregate` each build this context with a one-line closure (`inner -> build(dsl, table, bindings, inner)`, or `aliasBindings` for `having`) — the builder has **no knowledge of `SubqueryExpr`/`ComparisonNode`/`IN`** and does no filter-tree walking; compiling a subquery is a plain recursive `build(...)` call, invoked lazily by the translator exactly when it reaches the `IN` node (so nested subqueries — one whose own filter contains another — resolve for free). Constraints (all → 400): **same entity** as the enclosing query (reuses its table + bindings), and `subquery` valid **only** as an `in` right operand (`ExprTranslator` keeps a defensive reject). The 2-arg `toCondition(node, bindings)` stays for non-DSL callers (`QueryDslRunnableTestCaseSelector`) — a `null` context means a subquery operand is rejected. `QueryParameterResolver` recurses into `SubqueryExpr`.
