...
- **Multi-step conversation (`multiStep`) — array-driven turns** — when a DEPLOYMENT suite has `multiStep == true`, it uses its **single regular `inputBindings`** (no separate per-turn binding config). Per-turn variation comes from the **data**: the test-case columns bound by those bindings hold **arrays**, and `MultiStepConversationExecutor` (delegated from `EvaluationWorker` on `context.isSnapshotMultiStep()`) iterates them — turn `i` resolves the template with element `i` of each array-valued bound column (scalar columns and `constantValue` bindings broadcast unchanged). **Turn count is a per-test-case concern**: `N` = the common length of the array-valued bound columns *in that test case's data*, so two test cases in the same suite can run different turn counts. On a bad shape (no array-valued bound column, mismatched array lengths, or `N > MAX_CONVERSATION_STEPS`) the executor **fails only that test case** (persists one `ERROR` `TestCaseRunResult` and returns without throwing) — other test cases proceed. Each turn re-sends the full accumulated `messages` history; the assistant reply appended after each turn is the **full `choices[0].message` object, appended verbatim** (preserving `tool_calls`/`refusal`/structured content/etc.), never a reconstructed `{role, content}` — a turn aborts only when the 2xx response carries **no `choices[0].message` object** (a message with no string `content`, e.g. a tool-call turn, is valid). Because the response body is serialized with the shared `NON_NULL` mapper before the message is read, a reply's literal `content: null` is appended as absent `content`. Config-time validation (`SuiteValidationService`) for `multiStep` is just the normal `bindingValidator` run plus a chat-completions body check (top-level `messages` array); array-shape/length is a runtime-only concern. The executor persists `TestCaseRunResult.extractedColumns` as a **column-major JSON object** mapping each response column name to an array of its per-turn extracted values (single-step stays a JSON **object** of scalars); a step whose extraction fails contributes a `null` element (indices stay aligned), and a conversation that completes no steps yields `{}`. `TestCaseRunResult.extractionWarnings` aggregates every completed turn's extractor warnings (the *same* warnings the single-step path records — absent response body, JSONata/type-reconciliation failure; a legitimate null/no-match is **not** a warning, matching single-step) into one flat array, each warning stamped with a 0-based **`stepIndex`** (`MultiStepWarningAccumulator`, mirroring `MultiStepColumnAccumulator`; warning production reuses `ResponseColumnExtractor`, ser/deser reuses `ValidationWarningsSerializer`). `ExtractionWarningDto.stepIndex` is nullable and `NON_NULL`-omitted, so single-step warnings stay byte-identical (no `stepIndex` key); the `dataError` path (bad data shape, no turn ran) keeps `[]`. `responseBody` holds the **last turn's raw response body** (technical fields preserved, e.g. `id`/`usage`), mirroring `requestBody` which holds the last turn's raw request — the full conversation is recoverable from the last request body (it carries the whole message history through the final user turn) plus this final response. **There is no metric-boundary normalization** (`ExtractedColumnsNormalizer` was removed): metric bindings resolve against the **raw** stored `extractedColumns`, and `EvalSummary.extractedColumns` stores the arrays verbatim (the export CSV renders an array cell as compact JSON via the existing cell-serialization rules). A `ResponseBindingSourceDto` **or `TestCaseBindingSourceDto`** selects a specific turn/element via its optional **`jsonataExpression`** (in `BindingResolver`, evaluated against the resolved column value: `$[0]`/`$[-1]` pick a turn, object paths index a JSON-object column; **no** expression → the whole value/array; an expression matching nothing → `null`, since per-test-case turn counts make a fixed index legitimately out of range). This lets a metric score, e.g., a specific turn's **user message** from an array-valued test-case data column against that turn's answer. `ConstantBindingSourceDto` has no `jsonataExpression`. Invalid `jsonataExpression` syntax (on either binding type) surfaces as an `INVALID_EXPRESSION` soft TSMD validation warning (`MetricDefinitionValidationService`), not a hard 400. `multiStep` is an additive `SuiteSnapshotDto` field (version stays `"2"`).
- **Conditional metric execution (`condition` on TSMD)** — a TSMD carries an optional `condition` string (`test_suite_metric_definitions.condition VARCHAR(2000)`, nullable ⇒ always run) deciding whether that metric runs **per test-case result** (per-conversation granularity; per-turn is a future change). `ConditionExpressionEvaluator` (`service.domain`, single entry point) evaluates it against a **namespaced dictionary** `{ "data": <test-case columns>, "response": <extracted columns> }` — same `data`/`response` tokens as the CSV export, built as an `ObjectNode` and serialized **preserving explicit nulls** (never the shared `NON_NULL` mapper, which would drop a present-null column and make it look absent). Detection: a whole trimmed **bare `name()`** (regex `^[A-Za-z_][A-Za-z0-9_]*\(\)$`) is a custom/system function resolved via `ConditionFunctionRegistry` (a `ConditionFunction` SPI mirroring `QueryFunctionRegistry`, **shipped empty** — `isLastTurn()` etc. arrive with the per-turn change); anything else is JSONata via `JsonataEvaluationService` (JSONata functions are `$`-prefixed, so bare `name()` never collides). Custom functions and `evaluate(condition, ConditionContext)` take an **extensible `ConditionContext`** carrier (builder-backed) so future per-turn fields add without signature churn. **Write time** (`TestSuiteMetricDefinitionService.create/update`): `validate(condition)` → **hard 400** on invalid JSONata syntax or an unregistered `name()`; the condition does **not** feed the soft `is_valid` flag and is not persisted as a warning. **Run time** (`InProcessMetricEvaluationExecutor.evaluateAndBuild`, synchronously on the orchestrating thread before any async dispatch): clean boolean `true` → run; clean boolean `false` → **omit the metric entirely** (no `metricValues`/`metricInfos` entry — "absent = intentionally skipped"); any other outcome (throws / non-boolean / null) → new `TsmdEvaluationResult.ConditionError` → **no** `metricValues` entry + a **wholesale** metric-level `metricInfos[tsmd] = {error}` (renders as the `metricError::<name>` export column, relying on `error` not being an output-schema field name). `checkForErrors` **ignores** `ConditionError`, so a bad condition keeps the test-case result `SUCCESS` (never inflates failure counts). Phase-3 aggregate exclusion of a skipped metric is emergent (absent key → JSONB NULL → NULL-skipping SQL aggregates), not a code change.
