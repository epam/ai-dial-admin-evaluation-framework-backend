...
| [Multi-Request Chain](docs/patterns/request-chain.md) | `ChainNormalizer` as the single definition of "the chain"; chain-wide response-column namespace and the chain-union set; `responseField` accumulating-map resolution; fail-fast; condition-based metric targeting |
...
- **Rate limiting is per-HTTP-call, not per dispatch** — the `RunRateLimiter` gate on `EvaluationContext` is acquired inside each single-attempt call site (`EvaluationWorker.invokeSingle`, `EvaluationWorker.invokeMcpSingle`, `DeploymentTurnInvoker.invokeSingle`), and **retries consume tokens**. Do NOT reintroduce a per-dispatch `consume(1)` in `InProcessEvaluationExecutor`: that counted dispatches rather than requests, so a test case emitting N calls (multi-turn turns, chain requests, retries) put N× the configured RPS on the deployment. Call sites use `RunRateLimiter.tryAcquire()`, the single home for the interrupt policy: it re-sets the thread's interrupt flag and returns `false` so the caller skips its call without propagating a checked exception (`acquire()` remains the throwing primitive). An interrupted wait must leave **no** result row — `InProcessEvaluationExecutor` enforces that by discarding the worker's rows when the interrupt flag is set after `execute(...)` returns, since the gate reports interruption as a normal ERROR return value and every row-writing path below it would otherwise persist a synthetic row for a cancelled case.
...
- **Multi-request suites — an ordered chain, normalized once** — a suite may issue an ordered chain of independent HTTP requests per test case. Persisted **asymmetrically** (request 0 in the flat `endpoint_ref`/`request_template`/`input_bindings`/`response_columns` + `request_label`; requests `1..N-1` in `additional_requests` JSONB), then normalized by `ChainNormalizer` (`service.domain`) into a uniform `List<RequestSpec>`. **Every consumer works against the normalized list** — validation, execution, `request_index`/`request_label` assignment, the chain-union response-column set, `EvalSummaryExportColumnPlanner`, `EvalSummariesSchemaProvider` — and the same normalizer is applied to a live suite **and** a frozen snapshot, so never write `if (additionalRequests != null)`. A single-request suite normalizes to a one-element chain; `chain.size() > 1` is the only multi-request discriminator. Response column names are unique **chain-wide**, so the suite's effective set is the **chain union** (`chainResponseColumns` / `chainResponseColumnsJson`) — TSMD reference validation, export headers, and query-DSL `response:` fields MUST use the union, never the flat list. `InputBindingDto` gains a third source `responseField` (exactly one of `dataField`/`constantValue`/`responseField`) resolving from an accumulating map of earlier requests' extracted columns; each row still carries only **its own** request's `extracted_columns`. Fail-fast: the first failing request writes one ERROR row and aborts. Metric targeting reuses `condition` via the `request: {index, label}` namespace (`label` preferred — reorder-safe); an **unconditioned** metric runs on every row, and one bound only to test-case data/constants runs N× with **no** failure signal. `request_index` is in both analytics unique keys; `request_label` is not (mutable display value); there is **no** `total_requests`. Multi-request × multi-turn is rejected at **run creation** (409), so every multi-request row carries `turn_index=0`/`total_turns=1`. MCP chaining has a real seam (`ChainStepExecutorRegistry`) but is rejected at save (400) with a throwing stub. Cap: `test-suite.multi-request.max-requests` (default 10), enforced at save **and** run creation. See [request-chain pattern](docs/patterns/request-chain.md).
