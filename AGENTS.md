...
- **Query DSL `ParamExpr`** — `param` expressions ARE supported (do not re-reject them), resolved by a **single pre-pass** (`QueryParameterResolver` in `experimental.query.service.translate`) that rewrites a `StructuredQuery` into a parameter-free copy *before* translation — substituting each `ParamExpr` with its bound `Expr` recursively (unbound → 400; param-to-param → 400; cyclic chain → 400). The resolver is invoked once at the `StructuredQueryService.execute(query, params)` entry; the translator/builder/executor/repository are **parameter-agnostic** (do NOT re-introduce a `Map<String,Expr> params` argument threaded through them — that was the prior shape and was deliberately removed). `ExprTranslator` keeps a defensive `case ParamExpr → 400` since a surviving param means it was unbound. Internal callers use `StructuredQueryService.execute(query, params)`; the public `POST /api/v1/queries/execute` stays **paramless** by design (empty map → resolver is a no-op → any `param` is rejected by the translator guard → 400). The `metric-score-statistics` Phase-3 computation uses this param path (`StructuredQueryService.execute(query, params)`) to run the built-in statistic queries. Those queries (AVG/P10/P90/MIN/MAX and the default `overall`) are **code-defined** as typed `StructuredQuery` objects in `BuiltInMetricStatistics`. Because the executor depends on `experimental.query.service`, it **lives in** `experimental.query.service.metricscore` (implementing the `MetricScoreComputation` interface declared in the stable `service.domain.job` layer); `TestSuiteEvaluationJob` triggers Phase 3 through that interface, so there is **no** `service → experimental.query.service` bytecode edge and `LayeredArchitectureTest` stays unmodified. When wiring a stable-layer trigger to experimental code, invert via a `service`-layer interface — do not relax the layering test.
- **Query DSL function catalog is registry-driven** — DSL functions are NOT a hardcoded switch. Each is a `QueryFunction` bean (SPI in `experimental.query.service.translate.function`) collected by `QueryFunctionRegistry`; `ExprTranslator` delegates to the registry. To add a function, drop in a new `@Component QueryFunction` (or a `@Bean` in `BuiltInQueryFunctions`) — no translator/registry edits. Built-ins live in `BuiltInQueryFunctions`. Duplicate names are rejected at startup; unknown names → `ValidationException` (400). Most built-ins wrap a jOOQ built-in directly, but a function's `Field<?>` result can also delegate to a **custom Postgres stored function** for computations `FunctionContext`'s single-`Field`-per-call contract can't express as pure jOOQ (e.g. multi-row ranking): `roc_auc(label, probability)` aggregates both columns via `DSL.arrayAgg(...)` and calls the stored function `roc_auc_score(double precision[], double precision[])` (analytics DB, `V1.11__CreateRocAucScoreFunction.sql`) via `DSL.function(...)` — usable anywhere a `FnExpr` is valid, including a suite's custom `overallScore` expression, with no changes to `StructuredQueryBuilder`/`StructuredQueryExecutor`.
- **`test_cases` query entity + suite `testCaseFilter`** — `test_cases` is a **complex** queryable entity keyed by `dataset_id` (`TestCasesSchemaProvider`); its flattened `data::<field>` typing is **dataset-specific**, so its execute path is **instance-aware**: `PostgresTestCaseQueryRepository` requires a top-level `dataset_id` equality filter (missing/non-UUID → 400), builds typed bindings via `TestCaseFieldBindingsBuilder`, and calls the `StructuredQueryExecutor` **precomputed-bindings overload that bypasses the per-`Table<?>` cache** (cache-backed `test_suites`/`eval_summaries` are unchanged). `co`/`nc` on an `ARRAY`-typed `data::<field>` translate to JSONB containment (§ Query DSL function catalog). A suite carries an optional `testCaseFilter` (JSONB, mirrors `overallScore`) **validated at write time** (`TestSuiteService` → 400 on unknown field / unbound suite). It is applied at run time — combined `is_valid AND NOT excluded AND filter` — via the inverted **`service.domain.job.RunnableTestCaseSelector`** interface (impl `QueryDslRunnableTestCaseSelector` in `experimental.query.service`, same inversion as `MetricScoreComputation`); `RunnableTestCaseCounter` (guard #4) and `SnapshotInputWriter` (snapshot phase) call it, so there is **no** `service → experimental` edge.
- **Multi-turn conversation — row-based, one result row per turn** — multi-turn is **emergent from the data, not a suite flag**: a conversation is an **ordered group of `test_cases` rows** (one row per turn) sharing a top-level `conversation_id` (`VARCHAR(36)`, nullable, **client-supplied UUID**) with contiguous `turn_index` (`INTEGER`, nullable) from `0`; **both NULL ⇒ single-turn** (backward compatible). The two fields are **top-level** on the test-case request/response/batch-put DTOs (never inside `data`, so never schema-validated or template-visible). Authoring is **client-managed**: per-row write validation (`ConversationFieldsValidator`, from `TestCaseService` create/update/patch/batch + `CsvImportService`) enforces **both-or-neither**, valid UUID, `turnIndex >= 0`, `turnIndex < MAX_CONVERSATION_TURNS` (→ 400); a duplicate `(conversation_id, turn_index)` → 409 (partial unique `uq_test_cases_conversation_turn (dataset_id, conversation_id, turn_index) WHERE conversation_id IS NOT NULL`). Contiguity/completeness is **NOT** checked at write (turns arrive across requests) — it is validated **once at snapshot**. CSV import/export round-trips `conversationId`/`turnIndex` as reserved headers. **Counting is per individual test case — NOT per conversation:** guard #4 / `RunnableTestCaseCounter.countRunnable` counts individual runnable rows (`is_valid AND NOT excluded AND filter`, conversation turns counted individually) via the row-level `RunnableTestCaseSelector.countRunnable`. There is **no** per-conversation grouping in the counting path (the former `countRunnableUnits` and the `GROUP BY … HAVING count(*) = count(*) FILTER(cond)` SQL were **removed**). **Conversation grouping + integrity happen ONLY at snapshot.** The suite `testCaseFilter` is applied **row-level, like disable** — a non-matching turn is simply not a survivor: a non-matching / disabled / invalid **tail** turn truncates the runnable prefix; a **middle** hole (turn 0 missing, gap, filtered-out middle turn, any invalid turn among survivors) ⇒ broken; **all turns non-survivors** (e.g. every turn disabled) ⇒ the conversation is **dropped entirely — no execution unit** (it is *not* a broken `0/0` row; disable/filter fully remove it from the run). `number_of_test_cases` is seeded coarsely with the guard row count at creation, then **overwritten at snapshot** with the authoritative **execution-unit** count (single-turn rows + assembled conversations, conversation = 1 unit) returned by `SnapshotInputWriter.writeInputs`.
  **Snapshot & execution:** `TestSuiteEvaluationJob.attemptSnapshot` is **orchestration only** (snapshot tx, suite/dataset load, snapshot-JSON serialize) and delegates all input-row writing to **`SnapshotInputWriter`** (`service.domain.job`). The writer is **idempotent** (clears leftovers via `TestCaseRunInputRepository.deleteByRunId` before writing), parses `disabledTestCaseIds` via `DisabledTestCaseIdsCodec`, writes runnable single-turn rows as length-1 units first, then pages by **distinct `conversation_id`** (never straddling a page) via `RunnableTestCaseSelector.loadRunnableConversationIdsPage` (conversations with ≥1 filter-matching turn; `SELECT conversation_id … [filter in WHERE] GROUP BY conversation_id ORDER BY min(created_at_ms) ASC, conversation_id ASC` — the `GROUP BY` is only for dedup + the `min` sort, **not** the removed all-turns-match `HAVING`), loads the conversation's **filter-matching** turns (`loadConversationTurns(…, filterJson)` — only the filter is applied in SQL), groups + orders them, and `ConversationAssembler.assemble` (returns `Optional`) resolves exclusion/validity/contiguity in memory — **exclusion first, then every rule applies to survivors only** (a disabled turn, invalid or not, has zero influence): **no survivors ⇒ `Optional.empty()`, the conversation is skipped (no row written)**; otherwise missing turn 0 / gap / duplicate index / any invalid **surviving** turn / surviving count > `MAX_CONVERSATION_TURNS` ⇒ **broken**. It freezes each surviving conversation into **one** assembled `test_case_run_inputs` row (`conversation_id`, `total_turns` = surviving count, `turns` JSONB `[{testCaseId, testCaseName, turnIndex, data}, …]`, `broken` flag); single-turn = length-1 (`conversation_id`/`turns` NULL); deterministic order `min(created_at_ms), conversation_id`. Dispatch stays **one worker task per input × runIndex** (`InProcessEvaluationExecutor`). `EvaluationWorker.execute` branches **per input**: `input.isBroken()` → **one** sentinel `0/0` ERROR row, **no model call** (run continues, not FAILED); else `input.getTurns() != null` → `MultiTurnConversationExecutor`; else the single-turn path (there is **no** `context.isSnapshotMultiTurn()` flag anymore). `MultiTurnConversationExecutor` iterates the **frozen turns** — each turn resolves the suite's single `requestTemplate`/`inputBindings` against **that turn's own scalar `data`** (no array projection), re-sends the full accumulated `messages`, and appends the **full `choices[0].message` object verbatim** (preserving `tool_calls`/`refusal`/etc.; a turn aborts only when the 2xx response carries **no** `choices[0].message` object). It emits **one `TestCaseRunResult` per surviving turn** carrying that turn's own `testCaseId`/`testCaseName`, `turn_index` (authored) / `total_turns` (= surviving `N`), scalar `test_case_data`/`extracted_columns`, the full accumulated `request_body`, that turn's raw `response_body`, and the shared conversation-span `trace_id`. **Fail-fast:** an abort at turn `k` persists `k` SUCCESS rows + 1 ERROR row (all `total_turns = N`). **Contract:** the resolved body must be JSON with a top-level `messages` array — a body lacking it fails **per-conversation at run time** as an ERROR row (there is **no** suite-validation multi-turn body check). **Scope:** HTTP-deployment only — an `MCP_TOOL` suite bound to a dataset with any conversation row is rejected at run creation with **409 INVALID_OPERATION** (cheap `EXISTS` via `RunnableTestCaseCounter.hasConversationRows`). `ResultBatchWriter.addResults(buffer, List<TestCaseRunResult>)` advances the completed-**conversation** counter by one per call (`notifyProgress`, denominator = runnable conversations × `numberOfRuns`). Metric evaluation is the **single-value path**: `BindingResolver` resolves each binding to the raw scalar column value (missing → fail-fast); `ConditionContext.turnIndex`/`totalTurns` come from the result row (grouping-derived). **Removed** (feature-branch cutover): the suite `multiTurn` flag + `SuiteSnapshotDto.multiTurn`, `TurnPlan`/`ConversationTurnPlanner` array projection, `SuiteValidationService.validateMultiTurnBody`, and the JSONata/array-binding metric machinery; snapshot version stays `"2"`.
  **Turn columns:** analytics tables `test_case_run_results` and `test_case_eval_summaries` carry `turn_index INTEGER NOT NULL DEFAULT 0` and `total_turns INTEGER NOT NULL DEFAULT 1`; their natural-key unique indexes include `turn_index`. Both fields are exposed on `TestCaseRunResultResponseDto`/`EvalSummaryResponseDto` (auto-mapped, same-named `int`) and as CSV export identity columns immediately after `runIndex`. `EvalSummaryBatchWriteItemDto`'s two turn fields are **nullable `Integer`** (defaulting to `0`/`1` in the mapper) so external single-turn callers of `POST /api/v1/analytics/eval-summaries` stay byte-compatible. **Within-conversation turn ordering is client-side**: every turn row of a conversation-run shares the conversation span's `trace_id` (each turn is now its **own** `test_case_id`), so the frontend groups by `(traceId, runIndex)` and sorts each group by `turnIndex`; the keyset `(created_at, id)` cursor spine is unchanged.
- **Conditional metric execution (`condition` on TSMD)** — a TSMD carries an optional `condition` string (`test_suite_metric_definitions.condition VARCHAR(2000)`, nullable ⇒ always run) deciding whether that metric runs **per test-case result** — with per-turn rows this is now **per turn** (each turn is its own result). `ConditionExpressionEvaluator` (`service.domain`, single entry point) evaluates every condition as **JSONata** (`JsonataEvaluationService`) against a **namespaced dictionary** `{ "data": <test-case columns>, "response": <extracted columns>, "turn": { "index", "total", "last" } }` — `data`/`response` reuse the CSV-export tokens; `turn` exposes the result row's grouping-derived position (`index` 0-based, `total` the conversation's surviving turn count, `last` = `index == total - 1`; single-turn ⇒ `0/1/true`) so a condition can gate on turn position, e.g. `turn.last` to run only on the final turn, or `response.score > 0.5 and turn.last`. The dictionary is built as an `ObjectNode` and serialized **preserving explicit nulls** (never the shared `NON_NULL` mapper, which would drop a present-null column and make it look absent). `turn` is built from `ConditionContext` (builder-backed extensible carrier: `dataJson`, `responseJson`, `turnIndex`, `totalTurns`, populated in `InProcessMetricEvaluationExecutor` from the result row). **There is no custom-function form** — a prior design routed a bare `name()` to a `ConditionFunctionRegistry`/`ConditionFunction` SPI; that was **removed** (may return later). **Write time** (`TestSuiteMetricDefinitionService.create/update`): `validate(condition)` → **hard 400** on invalid JSONata syntax; the condition does **not** feed the soft `is_valid` flag and is not persisted as a warning. **Run time** (`InProcessMetricEvaluationExecutor.evaluateAndBuild`, synchronously on the orchestrating thread before any async dispatch): clean boolean `true` → run; clean boolean `false` → **omit the metric entirely** (no `metricValues`/`metricInfos` entry — "absent = intentionally skipped"); any other outcome (throws / non-boolean / null) → new `TsmdEvaluationResult.ConditionError` → **no** `metricValues` entry + a **wholesale** metric-level `metricInfos[tsmd] = {error}` (renders as the `metricError::<name>` export column, relying on `error` not being an output-schema field name). `checkForErrors` **ignores** `ConditionError`, so a bad condition keeps the test-case result `SUCCESS` (never inflates failure counts). Phase-3 aggregate exclusion of a skipped metric is emergent (absent key → JSONB NULL → NULL-skipping SQL aggregates), not a code change.
...
| `.experimental.query.service.metricscore` | Phase-3 metric-score computation: `BuiltInMetricStatistics`, `MetricScoreComputationExecutor` (implements `service.domain.job.MetricScoreComputation`)                                                                                                          |
| `.experimental.query.service.translate.function` | Registry-driven DSL function catalog: `QueryFunction` SPI, `QueryFunctionRegistry`, `FunctionContext`, `BuiltInQueryFunctions`, `MeanFunction`                                                                                                        |
