...
`./gradlew generateJooq` (or `./gradlew generateClickHouseJooq` for an analytics table), the generated `NewEntitiesRecord` becomes available.
...
Uses the typed jOOQ DSL. After adding a migration and running the matching codegen task (`generateJooq` for meta, `generateClickHouseJooq` for analytics), the
