...
| `spring.ai.mcp.server.streamable-http.mcp-endpoint` | `SPRING_AI_MCP_SERVER_STREAMABLE_HTTP_MCP_ENDPOINT` | `/mcp` | No | `spring.ai.mcp.server.enabled=true` | HTTP path of the Streamable HTTP transport (also reused by `protocol=STATELESS`). Read by `SecurityConfiguration` via `@Value` to build the authentication matcher, so overriding this path can never leave the endpoint unauthenticated. |
| `spring.ai.mcp.server.streamable-http.disallow-delete` | `SPRING_AI_MCP_SERVER_STREAMABLE_HTTP_DISALLOW_DELETE` | `false` | No | `spring.ai.mcp.server.enabled=true` | When `true`, disallows the `DELETE` method (session termination) on the Streamable HTTP endpoint. |
