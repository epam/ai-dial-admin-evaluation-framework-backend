...
When `datasource.analytics.vendor=CLICKHOUSE`, the analytics database is ClickHouse instead of PostgreSQL. The four analytics tables above (`test_case_run_results`, `test_case_eval_summaries`, `run_metric_snapshots`, `metric_score_result`) exist with **identical column names and semantics**; only the physical column types, engine, and dedup strategy differ. Full DDL: [`db/migration/analytics/CLICKHOUSE/V1.1__Init.sql`](../src/main/resources/db/migration/analytics/CLICKHOUSE/V1.1__Init.sql). These files are applied by Flyway (`flyway-database-clickhouse`, requires clickhouse-jdbc >= 0.10.0 and a `jdbc:clickhouse://` URL), the same as the PostgreSQL migrations — Flyway owns versioning and the `flyway_schema_history` table, and each script runs exactly once. `V1.1__Init.sql` still uses `CREATE TABLE IF NOT EXISTS` for transition safety (environments that ran the old hand-rolled schema initializer have the tables but no history row); new migrations (V1.2+) do not need that pattern.

- **Engine**: all four tables use `ReplacingMergeTree`, which is ClickHouse's stand-in for PostgreSQL's `UNIQUE` constraint + `ON CONFLICT DO NOTHING`. There is no upsert primitive — repositories always `INSERT`, and duplicate rows sharing an `ORDER BY` key are collapsed at read time rather than at write time (background merges are eventual; the analytics datasource pins the `final=1` server setting as a connection property so every read behaves as if `FINAL` were specified, forcing deterministic dedup even before a merge runs).
- **ORDER BY** (ClickHouse's primary/sorting key, standing in for PostgreSQL's unique-constraint columns): `test_case_eval_summaries` orders by `(test_suite_run_id, computation_id, test_case_id, run_index, request_index, turn_index, created_at_ms)`, the same column set as its PG unique constraint. `test_case_run_results` orders by `(test_suite_id, test_suite_run_id, test_case_id, run_index, request_index, turn_index, created_at_ms)` — a **superset** of its PG unique key `(test_suite_run_id, test_case_id, run_index, request_index, turn_index, created_at_ms)`: the leading `test_suite_id` has no counterpart in the PG constraint. The two keys currently partition the table into identical dedup groups only because `test_suite_id` is functionally dependent on `test_suite_run_id` (a run always belongs to exactly one suite); a future feature that reassigns a run's `test_suite_id` would break that dependency and must revisit this `ORDER BY`. Both tables are additionally `PARTITION BY toYYYYMM(fromUnixTimestamp64Milli(created_at_ms))`. `run_metric_snapshots` orders by `(computation_id, tsmd_id)`; `metric_score_result` orders by `(test_suite_run_id, computation_id, metric_score_name, metric_name)` — both matching their PG natural-key unique constraints exactly.
- **String, not JSON**: every JSONB column on PostgreSQL (`test_case_data`, `request_body`, `response_body`, `extracted_columns`, `extraction_warnings`, `metric_values`, `metric_infos`, `config_bindings`, `input_bindings`, `output_schema`, `log_details`) is a plain `String`/`Nullable(String)` column here, not ClickHouse's native `JSON` type — the application's serialized payload is stored byte-for-byte rather than being re-parsed and re-serialized by the database. The native `JSON` type was evaluated and rejected (verified on 25.8): it drops explicit `null` entries (load-bearing here — null vs absent distinguishes "metric failed" from "metric conditionally skipped"), flattens dotted keys into nested paths, fails the whole INSERT on a path collision, and its typed subcolumn reads return NULL when the stored variant differs (e.g. integer-valued scores under `.:Float64`).
- **`metric_values_map` acceleration twin** (`test_case_eval_summaries` only): a `Map(String, Map(String, Nullable(Float64)))` column `MATERIALIZED JSONExtract(metric_values, …)` — the JSON text parsed once at insert into typed columnar storage. All two-level numeric metric-path reads (the aggregate endpoint, `JSONB_NUMERIC` filters, the query DSL's `metric::<name>::<field>` family) read `metric_values_map[metric][field]` with bound-parameter keys instead of re-parsing the JSON blob per row; `metric_values` remains the serving source of truth and is what DTOs return. The twin is ClickHouse-only and deliberately absent from both generated jOOQ models (excluded in `generateClickHouseJooq`, carved out of `ClickHouseSchemaDriftTest`), so `AnalyticsModelParityTest` is unaffected; it is addressed by name in `DialectAwareJsonPathAccessor` and `ClickHouseEvalSummaryRepository`.
