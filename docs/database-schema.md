...
| `avg_metric_eval_duration_ms` | BIGINT | NOT NULL | 0 | Average latency (ms) of the TSMD provider `/evaluate` calls dispatched for this row's computation (V1.16); excludes TSMDs with a condition error (no call made) |
| `response_status_code` | INTEGER | NULL | - | HTTP response status code |
...
| V1.16 | `V1.16__AddAvgMetricEvalDurationToEvalSummaries.sql` | Added `avg_metric_eval_duration_ms` (BIGINT NOT NULL DEFAULT 0) to test_case_eval_summaries |
