...
Built on `spring-ai-starter-mcp-server-webmvc` (Spring AI 2.0.1) over the MCP Streamable HTTP protocol, sync server, tools capability only. Default `spring.ai.mcp.server.protocol=STATELESS` (one plain `application/json` JSON-RPC response per POST, no sessions, replica-agnostic); `STREAMABLE` (SSE responses, in-memory `Mcp-Session-Id` sessions) is a supported per-environment switch. Both are functionally tested. Configuration: [`docs/configuration.md` §2.6](../configuration.md#26-mcp-server-inbound).
...
1. Spring AI's servlet sync-server autoconfiguration calls `immediateExecution(true)` for `type=SYNC` — `McpServerAutoConfiguration.servletMcpSyncServerCustomizer` for `STREAMABLE`, and `McpServerStatelessAutoConfiguration` on the `StatelessSyncSpecification` for `STATELESS` (both verified in the Spring AI 2.0.1 bytecode) — so the sync tool specification does **not** add `subscribeOn(Schedulers.boundedElastic())`.
2. Both WebMVC transports handle a POSTed request synchronously on the container request thread: `WebMvcStatelessServerTransport` calls `handler.handle(msg).contextWrite(...).block()` and returns the JSON body; `WebMvcStreamableServerTransportProvider` does the same inside its `SseBuilder` consumer.
...
## `JsonMapperConfiguration.canRead(String)` / `canWrite(String)` gotcha
...
**Write side (`STATELESS`).** `WebMvcStatelessServerTransport` pre-serializes each JSON-RPC response to a `String` and returns it via `ServerResponse.body(String)` as `application/json`. With the Jackson converter ahead of `StringHttpMessageConverter`, `canWrite(String)` is `true` and the already-serialized JSON is re-encoded as a JSON *string literal*; MCP clients then fail with "expected object, received string" (seen in the official MCP Inspector). `canWrite` therefore declines `String.class` as well. `STREAMABLE` never hit this because it answers POSTs over SSE. `McpServerStatelessFunctionalTests` asserts the raw `initialize` body is a JSON object and that `tools/list`, the caller-identity probe and `get_deployment` behave identically under `STATELESS`, so the protocol switch is a pure configuration change (D11).
...
Default `protocol=STATELESS`: no `Mcp-Session-Id`, every POST is independent, GET → 405, no server→client notifications — the tools-only server uses none of these, and any number of replicas can sit behind a plain load balancer. Switching to `protocol=STREAMABLE` (`SPRING_AI_MCP_SERVER_PROTOCOL`) enables SSE responses and in-memory per-JVM sessions; that mode needs a single replica or sticky sessions routed on `Mcp-Session-Id`. Both modes are functionally tested (`McpServerStatelessFunctionalTests`, `McpServerFoundationFunctionalTests` pinned to `STREAMABLE`); see the `canWrite(String)` gotcha above for the converter dependency of `STATELESS`.
