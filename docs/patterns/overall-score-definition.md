...
**Future improvement**: today a population-dependent `CustomFunction` (`roc_auc` and similar) is silently accepted as `testCaseOverallScore`/`overallScore` and simply degrades to `NULL` per row at Phase-2 write time — there is no write-time signal that the chosen function can never produce a meaningful per-row value. A follow-up change could have the suite write path (`TestSuiteService`/`SuiteValidationService`) detect known population-dependent functions (starting with a denylist keyed by function name, e.g. `roc_auc`) at suite create/update time, against whichever definition actually drives Phase 2's per-row scoring — that is:
- `testCaseOverallScore` is population-dependent (whether or not `overallScore` is also set), or
- `testCaseOverallScore` is absent and `overallScore` (its fallback, per the table above) is population-dependent,

and surface a soft `validationWarnings` entry for that case — not a hard 400, since the run-level `overall` aggregate in `metric_score_result` remains perfectly valid for the same function; only the *per-row* reuse is meaningless. The warning message should point the author at the fix: set a distinct, row-safe `testCaseOverallScore` (e.g. `Mean`/`WeightedMean`, or a row-safe `CustomFunction`) so `overallScore` can keep using the population-dependent function for the run-level aggregate while per-row scoring uses something else. A `testCaseOverallScore` that is itself row-safe SHOULD NOT be flagged merely because `overallScore` is population-dependent — the two scopes are independent once `testCaseOverallScore` is explicitly set.
