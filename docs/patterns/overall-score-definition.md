...
`experimental.query.service.metricscore.OverallScoreDefinitionResolver` (a plain same-package collaborator of `MetricScoreComputationExecutor`, no interface inversion needed) turns the typed definition into a `StructuredQuery` at Phase-3 computation time:
