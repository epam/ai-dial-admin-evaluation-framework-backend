 ...
 * {@link com.epam.aidial.evaluation.runner.client.mcp.McpToolInvoker}.
 *
 * <p>Consumes the {@code "dialCoreTryOutRestClient"} bean by qualifier name; the {@code @Configuration}
 * class that declares it ({@code DialCoreDeploymentInvokerConfiguration}) stays in the EF backend, since
 * it composes interceptors defined there. This is a runtime-only wiring dependency (bean lookup by name),
 * not a compile-time one, so it does not violate the shared module's no-EF-backend-import rule.
 */
@Slf4j
@Component
@LogExecution
