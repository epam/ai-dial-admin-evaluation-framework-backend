...
**Known limitation, verified.** An `Api-Key` caller's principal is a `String` (`ApiKeyAuthenticationToken`), not a `Jwt`, so `McpCallerContext.jwt()` is `null` → `createdBy=anonymous`; it also has no `Authorization: Bearer` header for `AuthorizationHeaderInterceptor` to capture, so `bearerToken()` is `null` too and Core-backed tools call DIAL Core without caller credentials for that caller — exactly as REST controllers do today. This mirrors REST's own limitation and is out of scope for `mcp-foundation`; it is closed for both REST and MCP by the child change `api-key-core-propagation` (rollout order: right after `mcp-foundation`), see that change's own design for how.
