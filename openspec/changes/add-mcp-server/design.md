...
- Multi-replica session affinity for `STREAMABLE` (the default `STATELESS` protocol needs none).
...
`SecurityConfiguration` adds `.requestMatchers(mcpEndpoint).authenticated()` beside `/api/v1/**`, where `mcpEndpoint` is read **verbatim** from `spring.ai.mcp.server.streamable-http.mcp-endpoint` (default `/mcp`) — the same raw value the `STREAMABLE`/`STATELESS` transports register their single route on — so overriding the path can never leave the endpoint outside the authenticated matcher. No `/**` suffix (neither transport serves a sub-path) and no normalisation (PathPattern treats `/mcp` and `/mcp/` as different paths; stripping a trailing slash on one side only would send `/mcp/` to `denyAll`). Verified by `McpServerCustomEndpointOidcFunctionalTests` on `/agent/mcp/`. `NoSecurityConfiguration` already permits everything. MCP Origin-header validation is delegated to the Spring AI transport; verified by test.

**Verified in the foundation child (`mcp-foundation`): the request-thread assumption holds.** Spring AI's servlet sync-server autoconfiguration (`immediateExecution(true)`, active for `type=SYNC` in both the `STREAMABLE` and `STATELESS` autoconfigurations) plus the WebMVC transports' synchronous `block()` on the request thread (`WebMvcStreamableServerTransportProvider` and `WebMvcStatelessServerTransport` alike) together mean a tool body runs on the same HTTP request thread as the security filter chain, so `SecurityContextHolder` and `AuthorizationTokenHolder` are populated exactly as for a REST controller — no propagation machinery was needed. This was verified empirically with a test-scope `probe_caller_identity` tool **before** any production tool was written (`McpServerFoundationFunctionalTests`/`McpServerSecurityFunctionalTests`; `createdBy=alice` under a JWT, `createdBy=anonymous` with no credentials). `mcp.support.McpCallerContext` is the single place that reads the `Jwt`/bearer token/`Authentication` for tool code; it treats `AnonymousAuthenticationToken` as "no authentication" so `none`-mode probing reports `authenticationPresent=false` correctly. `McpToolExecutor` runs every tool body through this context.
...
### D11. Session model
Default `protocol=STATELESS` (`SPRING_AI_MCP_SERVER_PROTOCOL`), `type=sync`: no sessions, one `application/json` JSON-RPC response per POST, replica-agnostic. `STREAMABLE` remains a supported per-environment switch (SSE responses, in-memory `Mcp-Session-Id` sessions per JVM, needs a single replica or sticky sessions). Decided 2026-09-21 after `STATELESS` was verified as a pure configuration change by `McpServerStatelessFunctionalTests`; it required `JsonMapperConfiguration.canWrite` to decline `String.class`, mirroring the `canRead` exclusion, because the stateless transport returns pre-serialized JSON-RPC strings through Spring MVC's converters (the official MCP Inspector otherwise fails with "expected object, received string"). `McpServerFoundationFunctionalTests` is pinned to `STREAMABLE` for its session-specific scenarios. Documented in `docs/patterns/mcp-server.md`.
...
- [In-memory sessions break behind a load balancer] → avoided by the `STATELESS` default; only relevant if an environment opts into `STREAMABLE`.
