...
- **WHEN** `config.rest.security.mode=oidc` and a request to `/mcp` carries a valid JWT (or valid `Api-Key`) with at least one allowed role
...
- **THEN** requests to `/mcp` SHALL be processed without authentication
