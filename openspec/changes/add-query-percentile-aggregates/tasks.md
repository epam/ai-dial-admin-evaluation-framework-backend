...
- [ ] 1.1 In `ExprTranslator.toFunction` (`experimental.query.service.translate`) add `percentile_cont`
      and `percentile_disc` cases delegating to a new private two-arg handler.
- [ ] 1.2 Implement the handler: require exactly two args (else `ValidationException`); parse arg0 as a
      ...
      field via `toField(...)`.
- [ ] 1.3 Emit `DSL.percentileCont(fraction).withinGroupOrderBy(orderField)` /
...
- [ ] 2.1 Unit (translate): render test asserting `percentile_cont(0.1, "metric:Accuracy:score")` emits
...
- [ ] 2.2 Unit: reject wrong arity, non-literal `fraction`, non-numeric `fraction`, and `fraction`
      outside `[0, 1]` — each a `ValidationException`.
- [ ] 2.3 Functional (`PostgresFunctionalTests$EvalSummaryStructuredQueryTests` or a new nested test):
...
- [ ] 2.4 Run: `./gradlew test --tests "com.epam.aidial.evaluation.experimental.query.service.translate.*"`
      and the eval-summary execution functional test; all pass.

## 3. Spec, docs, and quality gates

- [ ] 3.1 `./gradlew spotlessApply checkstyleMain checkstyleTest` clean.
- [ ] 3.2 On archive, `/opsx:sync` the delta into `openspec/specs/structured-query-model/spec.md`
      (new **Supported function catalog** requirement; MODIFIED validation + SQL translation). NEVER
      hand-copy the delta.
- [ ] 3.3 No `specs/README.md` status change needed (`structured-query-model` already Implemented);
      verify its summary still reads accurately after the function-catalog addition.
- [ ] 3.4 No AGENTS.md / config.yaml change (feature follows existing translator patterns; no new
