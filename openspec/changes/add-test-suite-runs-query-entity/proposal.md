...
- New simple queryable entity `test_suite_runs` (meta datasource) at `POST /api/v1/queries/execute` and in the schema discovery endpoints. Fields: every `test_suite_runs` column **except** `suite_snapshot`, `run_config` and `error_details`, plus virtual fields extracted from the run's own `suite_snapshot`:
  `suite_type`, `deployment_ref::id|name|version|type`, `mcp_deployment_ref::id|name|type|transport` (snapshot only — no fallback to `test_suites`; null when the run has no snapshot), and `metric_names` (`array`): the alphabetically sorted `tsmd_name`s of the run's **latest computation** in `run_metric_snapshots`, `[]` for a run with no snapshots.
...
- **Code (new)**: `query.service.repository.PostgresTestSuiteRunEntityResolver` (`@ConditionalOnProperty datasource.meta.vendor=POSTGRES`, `metaDsl`), `query.service.TestSuiteRunsSchemaProvider`. `table()` is a derived table: narrowed `test_suite_runs` projection + `jsonb ->> key` extractions via `JsonPathAccessor`, `LEFT JOIN LATERAL` to a per-run `jsonb_agg(tsmd_name ORDER BY tsmd_name)` over the latest computation (`coalesce(..., '[]'::jsonb)`).
...
- **API**: additive — one new value in `GET /api/v1/queries/entities`, new `GET /api/v1/queries/entities/schema/test_suite_runs`, `entity: "test_suite_runs"` accepted by `/queries/execute`. No REST DTO changes. OpenAPI examples for the schema endpoint updated to mention the new entity where enumerated.
- **Docs**: new `docs/patterns/test-suite-runs-query-entity.md` (lateral latest-computation join, why not a global `GROUP BY` derived table, why snapshot-only refs) linked from AGENTS.md; `openspec/specs/README.md` index entry.
- **Perf/risk**: the lateral subquery runs two index probes per visited run; with the composite index the `LIMIT 1` latest lookup is an index-only scan with no sort. A sort on a non-indexed run column still visits every matching run (same as the plain table today). The `DISTINCT ON` global-aggregate alternative is rejected because Postgres cannot push page ids into an aggregated subquery.
