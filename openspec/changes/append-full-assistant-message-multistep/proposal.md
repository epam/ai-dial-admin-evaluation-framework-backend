...
- Change the per-turn abort gate: abort the conversation with `ERROR` only when `choices[0].message` is **absent or not a JSON object** (missing `choices`, empty array, or no `message`). A present message object with `content: null` (a tool-call-only turn) is now **valid** and appended as-is.
- Store the appended assistant message as the raw Jackson `JsonNode`/`ObjectNode` (not a converted `Map`), so explicit JSON `null` (e.g. `content: null`) survives the shared `NON_NULL` `ObjectMapper` serialization.
...
- **Tests:** `service/domain/job/MultiStepConversationExecutorTest.java` — add a turn whose mock `choices[0].message` carries extra fields (`tool_calls`/`refusal`, and a `content: null` case) asserting the resent request body's history contains the message verbatim with nulls preserved; keep/confirm the "no message object → ERROR, partial history" abort test. Existing turn tests are unaffected (their mock message is already just `{role, content}`). Functional `MultiStepConversationRunFunctionalTests` needs no change.
