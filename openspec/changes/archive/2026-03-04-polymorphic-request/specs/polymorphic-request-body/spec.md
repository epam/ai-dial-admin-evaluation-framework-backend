...
- `value` (Object, required) — template placeholder (e.g., `"${{prompt}}"`) or constant value; for `file` type, resolves to a blob UUID
- `filename` (String, nullable) — optional; may contain `${{var}}` placeholders; for file parts, overrides the stored blob filename in the multipart Content-Disposition
...
- **WHEN** a form part has `type: "file"` and `value: "${{document}}"` where `${{document}}` resolves to a blob UUID
- **THEN** the system SHALL retrieve the file bytes from BlobStorage and include them as a file part in the multipart message
...
- `type` (enum: `text`, `file`)
- `resolvedValue` (Object) — resolved value (string for text, blob UUID for file)
...
- `MultipartFormDataRequestBodySerializer` — uses `MultipartBodyBuilder`, reads file bytes from `BlobStorage` for file parts, returns `SerializedBody(MULTIPART_FORM_DATA, multipartBody)`
...
- **WHEN** the resolved body is `ResolvedMultipartBodyDto` containing a file part with blob UUID
- **THEN** `MultipartFormDataRequestBodySerializer` SHALL read file bytes from `BlobStorage.retrieve(blobId)`, build a multipart body via `MultipartBodyBuilder`, and return a `SerializedBody` with Content-Type `multipart/form-data`
