...
- Controller: `web/controller/TestCaseBulkDeleteController.java` — `@DeleteMapping("/api/v1/datasets/{datasetId}/test-cases:bulk")`
- Service method: `service/domain/TestCaseService.bulkDelete` — calls `ensureDatasetExists`, `bulkDeleteValidator.validate`, then `testCaseRepository.deleteByIdsAndDatasetId`
- Repository: `data/db/repository/PostgresTestCaseRepository.deleteByIdsAndDatasetId` — jOOQ `deleteFrom(TEST_CASES).where(...).returningResult(TEST_CASES.ID).fetch()`
- Ordering preserved by filtering input `ids` list against a `HashSet` of returned IDs (O(n), stable)
...
#### Implementation Notes
- `service/domain/TestCaseBulkDeleteValidator.java` — validates: not-null/non-empty ids, size ≤ cap, no null elements, no duplicate UUIDs
...
#### Implementation Notes
- `configuration/properties/testcase/TestCaseProperties.Bulk.maxDeleteIds` (`@Min(1)`)
- Default `max-delete-ids: 10000` in `application.yml`; doc row in `docs/configuration.md`
...
#### Implementation Notes
- `TestCaseService.bulkDelete` is annotated `@Transactional("metaTransactionManager")`; entire delete + response assembly runs in one transaction
