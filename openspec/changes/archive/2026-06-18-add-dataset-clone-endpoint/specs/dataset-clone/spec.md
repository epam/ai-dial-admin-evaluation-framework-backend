...
- Endpoint: `DatasetController.clone` (`POST /api/v1/datasets/{id}/clone`), `201 Created` + `ETag`.
- Request DTO: new `DatasetCloneRequestDto` with optional `name` (`@Size(max = ValidationConstants.MAX_DATASET_NAME_LENGTH)`, no `@NotBlank`) and optional `description` (`@Size(max = 2000)`).
- Orchestration: `DatasetService.clone` — pre-TX `DatasetCloneService.copyDatasetFiles`, then a `TransactionTemplate` (`metaTransactionManager`) with `TransactionTimestampContext` binding running `DatasetCloneService.cloneRowAndTestCases(source, newId, name, description, createdBy, timestamp, source.getVisibility())`; the returned id-map is discarded (no suite to remap). Best-effort `FileService.deleteAllByDatasetId` cleanup on failure; `UniqueConstraintViolationDetector.rethrowIfUniqueViolation` for 409.
- The clone's visibility and description are threaded through `DatasetCloneService.cloneRowAndTestCases` via new `String description` and `DatasetVisibility visibility` parameters (the suite-driven callers `TestSuiteService.detachDataset` and `TestSuiteCloneService` pass the source's description and `PRIVATE`, preserving their behavior).
- No DB schema change, no Flyway migration, no new configuration property.
