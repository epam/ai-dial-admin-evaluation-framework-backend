...
## 7. Read API — unified Query DSL entity only

- [x] 7.1 Expose `metric_score_result` as the `metric_score_results` queryable entity of the unified Query DSL (`MetricScoreResultSchemaProvider`, marker `MetricScoreResultQueryRepository`, `PostgresMetricScoreResultQueryRepository` on `analyticsDsl`; auto-registered). All 6 columns exposed; `in`/sort/aggregate supported; cross-computation reads via `computation_id in [...]`. Functional tests in `MetricScoreResultStructuredQueryFunctionalTests`.
- [x] 7.2 `computation_id eq "latest"` sentinel resolution in the entity repo: `MetricScoreLatestComputationDefaulter` (`@Component`, dep `ComputationResolver`), for a single-run query (`test_suite_run_id eq <uuid>`), extracts the single `computation_id` value and — when it is `"latest"` — resolves it via `ComputationResolver` (the single "latest" authority) and swaps the concrete id into the filter before translation. A real `computation_id` (eq `<uuid>`/`in`), an omitted `computation_id` (all computations), a non-single-run query, and a run with no computations are all left untouched. `PostgresMetricScoreResultQueryRepository.execute` applies it before delegating to `StructuredQueryExecutor`. Unit test `MetricScoreLatestComputationDefaulterTest` + functional `resolvesLatestSentinel`.
- [x] 7.3 No dedicated read endpoint: removed `MetricScoreResultController`, `MetricScoreResultResponseDto`, `MetricScoreResultMapper`, and `MetricScoreService.listResults` (+ its `resultMapper`/`computationResolver` deps); `MetricScoreService` keeps only `saveAll`. Dropped `MetricScoreComputationFunctionalTests.readsResultsViaLatest`.
