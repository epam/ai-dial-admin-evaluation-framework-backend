...
**Key decision — a subquery is compiled to a nested `SELECT` during translation (one SQL statement).** A subquery is literally "a query inside a query", so it maps onto the DSL directly: `FilterTranslator` emits `left IN (SELECT …)`, building the nested query through a compiler function supplied by whoever is translating. The execute-and-inline alternative (running the subquery separately and inlining its results as a literal array) was rejected — it produces two statements and re-types values; nested SQL is the SQL-native, atomic choice.
...
**8. Compilation via derived-table wrap, owned entirely by `FilterTranslator`.** `FilterTranslator` compiles each `IN`-subquery to `left.in(select(firstColumn).from(subselect.asTable()))`. The subquery's **first select column is the membership key**; remaining columns exist only to drive its `ORDER BY`/`LIMIT`, so `ORDER BY max(computed_at_ms) DESC LIMIT N` is expressed by selecting `max(...) AS recency` and sorting by `recency`. This keeps the sort grammar untouched (`IN` still receives a single column). An empty subquery result naturally yields no matches (`IN (SELECT … 0 rows)` is false) — no special handling.
...
**10. `FilterTranslator` compiles subqueries via a supplied `QueryCompiler` function — no cycle, no ambient state, no builder-side tree walk.** `QueryCompiler` (`@FunctionalInterface`, `StructuredQuery -> SelectQuery<Record>`) is a plain function value, never a Spring bean. `SubqueryContext(DSLContext dsl, String entity, QueryCompiler compiler)` bundles it with the enclosing entity and datasource, and is passed as an ordinary parameter to the 3-arg `toCondition(node, bindings, subqueryContext)`, threaded through the translator's existing recursive `(node, bindings)` methods — the same way `QueryParameterResolver` threads its own state. `FilterTranslator.subquerySelect` (the same-entity check, derived-table wrap, first-column extraction) is unchanged from its very first version — only the source of the nested `SelectQuery` changed, from `queryBuilderProvider.getObject().build(...)` to `ctx.compiler().compile(inner)`.

`StructuredQueryBuilder.build`/`countRows`/`buildAggregate` each construct a `SubqueryContext` with a one-line closure — `inner -> build(dsl, table, bindings, inner)` for `filter` (or `aliasBindings` for `having`, matching `having`'s existing alias-augmented field resolution) — and hand it to `filterTranslator.toCondition(...)`. The builder therefore needs **no knowledge of `SubqueryExpr`, `ComparisonNode`, `ComparisonOp.IN`, or the filter-tree shape at all**: it doesn't walk the tree looking for subqueries, it just supplies a way to compile an arbitrary `StructuredQuery`. Compilation happens exactly once per `IN` node, lazily, at the moment the translator's single recursive walk reaches it — nested subqueries (one whose own filter contains another) resolve for free, since the closure's `build(...)` call constructs its own `SubqueryContext` for the inner query's own tree.

`FilterTranslator` depends only on `ExprTranslator`; the builder→translator edge (`StructuredQueryBuilder` constructor-injects `FilterTranslator`) is the sole, one-directional dependency — ordinary constructor injection, no `ObjectProvider`, no `ThreadLocal`, no eager whole-tree pre-pass, no `IdentityHashMap`.

**11. Parameter resolver recurses into subqueries.** `QueryParameterResolver` rewrites `SubqueryExpr` by resolving params within its nested query, so a bound param inside a subquery substitutes correctly; the public paramless endpoint still rejects any surviving `param`.

**Component flow (read path, one request):** `POST /queries/execute` → `StructuredQueryService.execute(query, {})` → `QueryParameterResolver` (recurses subquery) → `repository.execute(...)` → `StructuredQueryExecutor` → `StructuredQueryBuilder.build` (constructs a `SubqueryContext` closing over `dsl`/`table`/`bindings`) → `FilterTranslator.toCondition(node, bindings, subqueryContext)` compiles each `IN`-subquery on demand via the supplied `QueryCompiler`. One SQL statement.
