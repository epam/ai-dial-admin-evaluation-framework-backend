# Metric Evaluation (delta: row-based-multi-turn)

This delta aligns metric evaluation with the row-per-turn conversation model. Because a
multi-turn conversation is now an ordered group of **test-case rows** (one row per turn)
rather than one row whose columns hold arrays, every `TestCaseRunResult` — single-turn or
per-turn — carries **scalar** `test_case_data` and **scalar** `extracted_columns`. Metric
binding resolution therefore stays on the simple per-turn scalar path with no array/JSONata
machinery, and one EvalSummary is produced per turn-result.

> Note: this spec never contained an array-aware / JSONata metric-binding requirement (that
> machinery lived in the multi-turn-conversation capability and AGENTS inline conventions),
> so there is nothing to REMOVE here. The two requirements below are MODIFIED to state the
> per-turn scalar contract explicitly and to reference the row model. Condition-gating
> semantics are owned by the conditional-metric-execution spec and are not restated here.
...
- `BindingResolver` (`service.domain.job.BindingResolver`) resolves each binding to the raw scalar column value from the per-turn `test_case_data` / `extracted_columns` maps — no JSONata step, no array/scalar mismatch guard, fail-fast on missing column.
- `MetricEvaluationWorker` (`service.domain.job.MetricEvaluationWorker`) evaluates a single TSMD against a single per-turn `TestCaseRunResult`; each turn is an independent result row, so metrics are evaluated per turn-result.
- `InProcessMetricEvaluationExecutor` (`service.domain.job.InProcessMetricEvaluationExecutor`) iterates all `TestCaseRunResult` rows (one per turn) and assembles one EvalSummary per turn-result, copying scalar `extractedColumns` and `turnIndex`/`totalTurns` verbatim; `ConditionContext.turnIndex`/`totalTurns` are sourced from the result row's turn position (now derived from conversation-row grouping, not array length).
