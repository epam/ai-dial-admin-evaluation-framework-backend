...
**Reason**: The array-per-column model is replaced by the row-based model. Turn count is no longer derived from the common length of array-valued bound columns on a single row; it is the surviving turn count of the ordered group of discrete rows keyed by `conversation_id`/`turn_index`. `ConversationTurnPlanner` and `TurnPlan` (array projection / per-field iteration) are removed.
**Migration**: Author each turn as its own `test_cases` row sharing a `conversation_id` with a distinct `turn_index` (contiguous prefix from 0), each row holding scalar `data`. The per-conversation turn count becomes `N` = number of surviving (valid, enabled, contiguous) turn rows. Over-cap is enforced at write time (`turnIndex < MAX_CONVERSATION_TURNS`) and at snapshot as a broken-conversation check, not from array length.

### Requirement: Data-shape failure yields a single degenerate ERROR row
**Reason**: The array-shape data errors (no array-valued bound column, mismatched array lengths across bound columns, per-row derived turn count over cap) no longer exist in the row model — there are no array columns to mis-shape. The degenerate `turn_index=0, total_turns=0` ERROR row now represents a BROKEN CONVERSATION (missing turn 0, gap/non-contiguous prefix, duplicate `turn_index`, any invalid turn, or surviving turn count over `MAX_CONVERSATION_TURNS`), detected at selection/snapshot time, not an array-shape mismatch at execution time.
**Migration**: Ensure each conversation's turn rows form a contiguous prefix `0..k` with all turns valid and enabled (tail-only disabling); the runnable selector includes a conversation only if all its turns match the `testCaseFilter`. Broken conversations are skipped and surfaced as one ERROR result row with sentinel `turn_index=0, total_turns=0` (see the test-suite-run selection spec), and the run continues.
