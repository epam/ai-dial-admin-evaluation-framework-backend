...
- [x] 1.1 Add Flyway analytics migration `src/main/resources/db/migration/analytics/POSTGRES/V1.15__AddLastTurnIndexToTestCaseRunResults.sql`: `ALTER TABLE test_case_run_results ADD COLUMN last_turn_index INTEGER NOT NULL DEFAULT 0;` (comment: max authored surviving turn index of the multi-turn; `0` for single-turn). Do NOT touch any unique/idempotency index (it already includes `turn_index`).
...
- [x] 2.1 `MultiTurnAssembler`: delete `isContiguousFromZero` and the `contiguous` term. Set `broken = anyInvalid || survivors.size() > MAX_MULTI_TURN_TURNS`. Keep survivors sorted ascending by authored `turnIndex`; keep the empty-survivors → `Optional.empty()` drop.
- [x] 2.2 `MultiTurnAssembler.AssembledMultiTurn`: add `int lastTurnIndex` (max authored surviving `turnIndex`; `0` when broken/empty is irrelevant since broken carries no turns). Populate it in `assemble`. `turnsJson` keeps authored `turnIndex` per turn (no renumbering).
- [x] 2.3 Run `./gradlew test --tests "*MultiTurnAssemblerTest"` after updating that test (see group 6).

## 3. Executor + worker — carry `lastTurnIndex`, reword sentinel

- [x] 3.1 `MultiTurnExecutor`: thread the multi-turn's `lastTurnIndex` (max authored surviving index across the frozen turns) into every `TestCaseRunResult` via `buildTurnRow` (keep `turnIndex` authored, `totalTurns` = surviving count). Set it on the degenerate/abort error rows too.
- [x] 3.2 `EvaluationWorker.buildBrokenMultiTurnResult`: set `lastTurnIndex = 0` on the `0/0` sentinel row and reword the `BROKEN_MULTI_TURN` message to reference only "an invalid turn, or too many turns" (remove turn-0 / contiguity / duplicate-index wording).
- [x] 3.3 Run `./gradlew test --tests "*MultiTurnExecutorTest" --tests "*EvaluationWorkerTest"` after updating those tests (group 6).

## 4. Write-time validation — drop the cap bound

- [x] 4.1 `MultiTurnFieldsValidator`: remove the `turnIndex >= MAX_MULTI_TURN_TURNS` → 400 check. Keep both-or-neither, valid UUID, `turnIndex >= 0`. Leave `MAX_MULTI_TURN_TURNS` in `ValidationConstants` (now enforced only at assembly).
- [x] 4.2 Run `./gradlew test --tests "*MultiTurnFieldsValidatorTest"` after updating that test (group 6).
...
- [x] 6.1 `MultiTurnAssemblerTest`: flip contiguity cases (missing turn 0, gap, middle hole) from broken→runnable with authored indices preserved; assert `lastTurnIndex`; keep invalid-surviving-turn and over-cap → broken; keep empty→`Optional.empty()`.
- [x] 6.2 `SnapshotInputWriterTest`: assert a middle-disabled / filtered-out multi-turn is materialized as a runnable ordered-turns input (not a broken marker); assert fully-disabled multi-turn is dropped.
- [x] 6.3 `MultiTurnExecutorTest`: assert emitted rows carry authored `turnIndex` and the multi-turn `lastTurnIndex`; add a non-contiguous survivors case.
- [x] 6.4 `EvaluationWorkerTest`: assert the broken sentinel carries `last_turn_index = 0` and the reworded message.
- [x] 6.5 `MultiTurnFieldsValidatorTest`: remove the over-cap 400 case; add a "large turnIndex accepted" case; keep both-or-neither / UUID / negative.
...
- [N/A] 7.2 `docs/patterns/suite-run-snapshot.md` contains no multi-turn/multi-turn content (verified via grep) — nothing to reword. The snapshot-integrity behavior is documented in AGENTS.md (7.1) and the delta specs instead.
