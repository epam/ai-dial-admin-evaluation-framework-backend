...
- [x] 3.6 Unit tests: `MultiTurnFieldsValidatorTest`, per-turn `TestCaseValidationService` cases; functional tests: multi-turn create/read/PATCH round-trip, CHECK-constraint & 400 paths
...
- [x] 4.4 Unit + functional tests: export multiplication, import assembly, round-trip, conflict strategies over assembled cases
...
- [x] 5.3 Functional tests: all-turns-match include/exclude, missing-field turn fails, single-turn parity, no-filter query unchanged
...
- [x] 6.4 Unit tests: `ConditionExpressionEvaluatorTest` (RUN/SKIP/ERROR, `turn.last`, present-null vs missing, single-turn); functional: condition round-trip + 400
...
- [x] 7.7 Unit tests: `MultiTurnExecutorTest`, `DeploymentTurnInvokerTest`, `DeploymentInvocationSupportTest`; functional: `MultiTurnRunFunctionalTests` (2-turn, fail-fast, non-chat body, MCP 409)
...
- [x] 8.4 Unit tests: condition-false not dispatched + SUCCESS, condition-error surfaced + SUCCESS, `MetricOutputMapper` ConditionError
...
- [x] 9.3 Functional tests: per-turn result & summary rows, turn-key uniqueness, single-turn batch-write defaults
...
- [x] 10.2 Update OpenAPI `@Schema`/example files for `multiTurnData`, `condition`, and turn fields on result/summary DTOs
- [x] 10.3 Sync delta specs into `openspec/specs/` and update `openspec/specs/README.md` per Spec Index Maintenance Policy (new folders `multi-turn-conversation`, `conditional-metric-execution`)
...
- [x] 10.5 Run `./gradlew spotlessApply checkstyleMain checkstyleTest test`; fix any violations
