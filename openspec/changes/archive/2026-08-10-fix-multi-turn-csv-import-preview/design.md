...
**Implementation note — MERGE-delta membership now matches preview's traversal.** `buildMergeDelta` determines "new field" by iterating CSV column *bindings* (as `buildAutoDetectedSchema`'s MERGE branch already did for preview) rather than iterating `inferredTypes` entries (as the old `persistSchema`/`buildFinalSchema` MERGE branches did). Type inference skips blank cells, so a new column that is blank on every row of the CSV never gets an `inferredTypes` entry: under the old per-path behavior it was silently omitted from the *persisted*/*fixup* MERGE schema while preview's `autoDetectedSchema` already reported it (using `SchemaFieldType.STRING` as the default). Routing all three MERGE branches through one builder makes that inconsistency visible and resolves it in favor of the preview's (correct, pre-existing) behavior: an all-blank new column is now consistently added to the schema as `STRING` everywhere. This is the same class of preview-vs-import divergence this change exists to close, not a new one.

### D2 — Run grouping via a stateless component plus a per-call accumulator

Streaming grouping is inherently stateful (the current run, and the set of multi-turn names already completed), but a Spring singleton must not hold per-request state. So: a stateless `@Component` (working name `CsvTestCaseGrouper`) exposes a factory for a small per-call accumulator object; the accumulator takes rows one at a time and emits a completed `CsvTestCase` when the `testCaseName` changes, plus a final `flush()`.
...
A multi-turn run's one name occurrence is registered through the same check as a single-turn row's. So a *second* run of an already-seen multi-turn name — necessarily non-contiguous, since adjacent identical names merge into one run — produces **both** the non-contiguity warning and the ordinary duplicate/collision warning: unlike turn rows within one run, it is a genuine second occurrence of the name and collides at persist time under every conflict strategy.

### D4 — A multi-turn assembler owns ordering, partitioning and conflict detection

New `@Component` (working name `MultiTurnRunAssembler`) takes a multi-turn `CsvTestCase` plus the validation schema and returns the assembled shape: ordered per-turn maps, the shared map, and the detected conflicts (shared-column mismatch, duplicate `turnIndex`, plus the JSON-parse flag the run's rows already carry). It absorbs today's private `orderTurns`, `assembleMultiTurn` and `hasDuplicateTurnIndex`, and delegates scope partitioning to the existing `TestCaseFieldScopeResolver` (unchanged).
...
`fixupTestCases` is a **recomputation** pass, so D8's carry-forward rule applies: the re-validated result is unioned with the stored `INVALID_INPUT` warnings and the case stays invalid while any remain. Without that, RC3 itself erases the import's conflict verdict before revalidation is ever reached — reachable today with `dup,0,1` / `dup,0,2`, where `inferCellType` makes the column `INTEGER`, coercion changes the stored values, and the re-validation fires.
...
- A small injectable merger in `service.domain` takes (recomputed result, stored warnings JSON) and returns the recomputed warnings unioned with the stored `INVALID_INPUT` entries, with `valid = recomputedValid && preserved.isEmpty()`. Both recomputation passes call it immediately before writing — which means `RevalidationService` gains it as a constructor dependency; there is no way to have an injectable merger *and* an unchanged constructor, and the merger is the part that must not be duplicated.
...
*Consequence, stated rather than hidden:* a `INVALID_INPUT` warning persists until the case is edited or re-imported. That is the intended lifetime — the case genuinely was authored from conflicting rows, and no amount of inspecting the stored data can disprove it.
...
## 6.3 — config.yaml / specs/README.md update-need check

Checked against `openspec/config.yaml`'s Config Maintenance Policy and Spec Index Maintenance Policy. Neither needs an update:

- **config.yaml**: the new `CsvSchemaFieldBuilder`/`CsvTestCaseGrouper`/`MultiTurnRunAssembler`/`ValidationWarningsSerializer`-merger components (plus their `CsvTestCase`/`ParsedCsvRow`/`ColumnBinding` carriers) all land inside the pre-existing `service.domain.csv` and `service.domain` packages, following the already-documented "specialized, injectable components for conversion/validation logic" convention (AGENTS.md Do's, and this change's own Non-Goals: "No new package, no new architectural layer, no change to the `service.domain.csv` package's role"). No new layer, no new cross-cutting convention, no tech-stack bump, no new tooling command. This is "follows the rules", not "changes the rules" — per the Config Maintenance Policy litmus test, that means specs/AGENTS.md, not config.yaml.
- **specs/README.md**: no new spec folder was added (this change only carries delta specs for the existing `multi-turn-test-case` and `test-cases` folders, synced at archive time per 6.2). Neither spec's status changes (both stay "Implemented"). The one-line summaries in `specs/README.md` (`multi-turn-test-case`: "Flat CSV multiplication (reserved `turnIndex` column, shared columns repeated, contiguous-run assembly on import)"; `test-cases`: "TestCase CRUD, PATCH, CSV export/import, schema validation, re-validation") both remain accurate after this fix — it corrects a bug in schema carry-forward and adds turn-aware preview/durable-conflict behavior, but doesn't add, remove, or rename a capability the summaries claim. No update needed.
