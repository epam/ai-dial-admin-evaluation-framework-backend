...
- [x] 1.1 Replace `appendDisabledTestCaseIds` in
...
- [x] 1.2 Add `src/test/java/com/epam/aidial/evaluation/functional/tests/LegacyDisabledTestCaseIdsFunctionalTests.java`
...
- [x] 1.3 Add a second scenario to the same class: the suite has a `testCaseFilter` matching only test cases
...
- [x] 1.4 Run `./gradlew :test --tests "*LegacyDisabledTestCaseIdsTests*"` and record both failures
...
- [x] 2.1 Drop `excludedIds` from `service/domain/job/RunnableTestCaseSelector` (`countRunnable`,
...
- [x] 2.2 Update `experimental/query/service/QueryDslRunnableTestCaseSelector` to the new signatures,
      delegating to the renamed repository methods from 3.1 (done: compiles against the new interface).
- [x] 2.3 Delete `service/domain/RunnableTestCaseCounter.java` and its unit test
...
- [x] 2.4 Remove the `deserializeDisabledIds` helper and its call site from
      `service/domain/TestSuiteRunService.java` (done: the guard reads only `datasetId` + `testCaseFilter`).
- [x] 2.5 Remove the `deserializeDisabledIds` helper and its call site from
      ...
      argument).
- [x] 2.6 Update `service/domain/job/InProcessEvaluationExecutor.fetchPage` legacy fallback to call
...
- [x] 3.1 In `data/db/repository/TestCaseRepository.java` + `PostgresTestCaseRepository.java`, rename
...
- [x] 3.2 Remove the `disabledTestCaseIds` field from `data/db/model/TestSuite.java` and stop mapping it in
      `data/db/mapper/TestSuiteRecordMapper.java` (done: the generated `TestSuitesRecord` getter is no longer
      called anywhere).
- [x] 3.3 In `data/db/repository/PostgresTestSuiteRepository.java`, delete the three
...
- [x] 3.4 Change `TestSuiteRepository.updateDatasetId` / its Postgres impl to
...
- [x] 3.5 (ran with group 5 — the test source set did not compile until group 5 landed) Run `./gradlew test --tests "*JooqSchemaDriftTest" --tests "*JdbcTemplateFenceTest"
...
- [x] 4.1 Remove `disabledTestCaseIds` (field, `@Size`, `@Schema`) from
...
- [x] 4.2 In `service/domain/mapper/TestSuiteMapper.java`, delete `serializeDisabledIds`,
...
- [x] 4.3 Remove the remap block from `service/domain/TestSuiteCloneService.java` (step "5pre") and from
...
- [x] 4.4 Change `service/domain/DatasetCloneService.cloneRowAndTestCases` to return `void`, deleting the
      `Map<UUID, UUID> idMap` accumulation and updating its javadoc (done: no per-test-case map is built;
      all three call sites compile).
- [x] 4.5 Update the descriptions that reference the field: the clone-endpoint text in
...
- [x] 4.6 Remove `disabledTestCaseIds` from the ~17 OpenAPI example files under
...
- [x] 5.1 Delete the tests whose subject is removed: `LargeDatasetSnapshotFunctionalTests` (10000-element
...
- [x] 5.2 Drop the vestigial `enabled` flag from `BaseTestCaseBulkPatchFunctionalTests.createTestCase` and
      ...
      fixture).
- [x] 5.3 Update the unit tests that build suites or stub the clone service:
      ...
      compile and pass).
- [x] 5.4 Rework `DatasetCloneServiceTest` cases that asserted the returned id map to assert the clone via
      captured repository inserts (new ids, repointed `datasetId`, `@ef/datasets/` ref rewrite) (done: the
      `void` signature is fully covered).
- [x] 5.5 Re-run `./gradlew :test --tests "*LegacyDisabledTestCaseIdsTests*"` (done: both scenarios
      from group 1 now PASS), then
      `./gradlew :test --tests "*TestSuiteRunTests*" --tests "*SuiteSnapshotTests*"
      --tests "*TestSuiteCloneTests*" --tests "*DatasetDetachTests*" --tests "*TestSuiteDatasetTests*"`
      (abstract `*FunctionalTests` classes only run as `PostgresFunctionalTests$<Nested>` — filter on the nested name, and scope to `:test` so `:eval-cli:test` does not fail on zero matches) (done: green).
- [x] 5.6 Run `./gradlew :evaluation-runner-core:test :eval-cli:test`, then `./gradlew spotlessApply` and
...
- [x] 6.1 Update `docs/patterns/dataset-entity.md` — remove `disabledTestCaseIds` as a headline concept and
      ...
      only).
- [x] 6.2 Update `AGENTS.md` — the Unique Patterns row for `Dataset Entity` lists `disabledTestCaseIds` in
...
- [x] 6.3 Update `docs/database-schema.md` — mark `test_suites.disabled_test_case_ids` as retained but
...
- [x] 6.4 Update the non-requirement prose that the delta specs cannot carry: the `test-suites` spec Key
      Terms bullet and the `test-cases` spec Purpose paragraph, both of which describe the field (done: both
      point at `testCaseFilter` instead). Also fix the stale symbol names left in main-spec Implementation
      Notes that the delta merge may not overwrite: `RunnableTestCaseCounter` (test-suite-runs:641,
      suite-test-case-filter:137) and `findValidByDatasetIdExcludingIds` (suite-run-snapshot:184,
      test-suites:126). Applied during `/opsx:sync` alongside the delta merge (done: Key Terms, Purpose, and the four stale symbol references updated).
- [x] 6.5 Update `openspec/specs/README.md` per the Spec Index Maintenance Policy — the `test-suites`,
...
- [x] 6.6 Confirm no `openspec/config.yaml` update is needed (no new layer, convention, tooling, or glossary
