...
- Each probe records a failure as an outcome rather than propagating it. `DialCoreClientException` is the client's *translated* failure, but `DialCoreClient.withRetry` catches only `RestClientResponseException` — transport failures (connect/read timeout, connection reset) and message-conversion failures escape it raw — so the probe also catches `RestClientException` and records it as a 502 outcome. Without that second catch, one unreachable endpoint discards a sibling probe's hit, because the three futures are joined in a single stream expression. `CompletableFuture.join` wraps anything escaping a task in `CompletionException`, so the join site unwraps one level before inspecting — skipping that unwrap would degrade every upstream failure into a generic 500. An `InterruptedException` surfacing during the join restores the interrupt flag before rethrowing.
...
The component decides purely from its inputs (it logs, but reads nothing else), so every branch in §5 and §6 is a plain unit test with no Spring context, no mocked HTTP, and no timing.
...
- Every `catch` logs with the exception as the last SLF4J argument (`LoggingConventionTest`-enforced). A losing leg's 404 is logged at DEBUG — it is the expected outcome on two of three legs — while any other status logs at WARN, whether or not the lookup as a whole succeeded, so a 401 or a 5xx on one leg stays visible in production rather than vanishing behind a 200.
...
### 9. OpenAPI example wiring for a trailing-wildcard mapping

`OpenApiExampleCustomizer` derives an example's filename from the registered OpenAPI path, and SpringDoc registers a trailing-wildcard mapping with the `/**` intact (verified against `/v3/api-docs`). The naive key for this endpoint is therefore `api-v1-deployments-all-**`, and `*` in a filename is illegal on Windows and glob-hostile everywhere. `pathToKey` now strips a trailing `/**`, so the key is `api-v1-deployments-all`.

That normalization also revealed why the sibling endpoints' example files were dead: they were named for a pre-wildcard path shape (`…-deploymentType-deploymentId-GET-…`) that stopped matching when PR #131 changed the mapping to `/{deploymentType}/**`, and nothing failed when the examples silently stopped being injected. They are renamed to their real keys, and an assertion on `/v3/api-docs` now pins the wiring for the new endpoint so it cannot rot the same way.
