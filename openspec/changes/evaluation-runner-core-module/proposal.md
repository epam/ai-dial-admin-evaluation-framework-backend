...
- `TestCaseRunner` is restructured from a stateless `run(list, ...)` call into a session-scoped scheduler (`submit(List<TestCaseRunInput>)` per page + `awaitCompletion()` once), constructed per run by a new `TestCaseRunnerFactory` (shared module). This restores the pre-extraction behavior of one continuous rate-limit bucket and continuously saturated concurrency for the whole run, correcting two real regressions introduced by calling `run()` once per DB page. See Decision 14 in `design.md`.
