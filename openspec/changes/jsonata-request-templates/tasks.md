...
- [ ] 1.1 Add a `Frame`-aware `evaluate(String expression, String jsonData, Frame frame)` overload (or
...
- [ ] 1.2 New `JsonataProperties` (`@ConfigurationProperties(prefix = "jsonata")`,
...
- [ ] 1.3 Add `jsonata.evaluation-timeout-ms` and `jsonata.max-recursion-depth` rows to
  `docs/configuration.md` (all six columns).
- [ ] 1.4 Unit test: an expression exceeding the configured bounds throws (reusing the spike's runaway
  expression); a normal expression is unaffected.

## WP2 — Request body model + resolver refactor

- [ ] 2.1 `JsonRequestBodyDto.content`: `Map<String, Object>` → `Object`. Update `@Schema`
  examples/OpenAPI to `oneOf` [object, string].
- [ ] 2.2 `JsonataSourcePreprocessor` (new, `service.domain`): textual `${{}}` placeholder substitution
...
- [ ] 2.3 `TemplateContentResolver` (new/refactor, `service.domain`): Map-content path (existing
...
- [ ] 2.4 `RequestBodyEvaluator` (new, `service.domain`): evaluates the resolved body text as JSONata
...
- [ ] 2.5 Wire `ResolvedRequestService`'s preview path (`GET .../resolved-request`) through
...
- [ ] 2.6 Functional/unit test: an existing Map-content suite with no JSONata-specific syntax produces a
...
- [ ] 3.1 `JsonataReservedNames` (new constants class, `constants` or `service.domain`): the JSONata
...
- [ ] 3.2 `TestSuiteRequestValidator`: reject (HTTP 400) a `String`-content request body that fails
...
- [ ] 3.3 `TemplateVariableExtractor` (or equivalent): unit test pinning that `String`-content placeholder
...
- [ ] 4.1 `DashjoinJsonataEvaluationService`/`JsonataEvaluationService`: add a 3-arg
...
- [ ] 4.2 `ResponseColumnExtractor.ExtractionResult`: carry through reconciled values in a form
...
- [ ] 4.3 `ResponseColumnExtractor.extract(...)`/`buildResult` pass-through: when a column's extraction
...
- [ ] 4.4 Unit test: `$request`/`$response` frame variables are reachable and structurally correct;
...
- [ ] 5.1 `DeploymentTurnInvoker`: remove the non-streaming-only constraint; invoke deployments with
  streaming enabled for every turn (reuse the single-turn SSE invocation path instead of the dedicated
  non-streaming call).
- [ ] 5.2 `TurnOutcome`: add `logDetails` (or equivalent) so a streaming turn failure carries the same
  diagnostic detail a single-turn SSE failure would, matching single-turn's error-envelope shape.
- [ ] 5.3 Remove `content.put("stream", false)` force-injection from the turn body-assembly path.
- [ ] 5.4 Unit/functional test: a multi-turn suite streams each turn and still produces the same persisted
...
- [ ] 6.1 `EvaluationContext`: add `snapshotTestCaseSchema` (`List<FieldDefinitionDto>`, sourced from
  `SuiteSnapshotDto`, builder field like existing `snapshot*` members).
- [ ] 6.2 `PerTurnBindingDetector` (new, `service.domain.job`): given the effective `inputBindings` and
...
- [ ] 6.3 `TurnLoopExecutor` (new, `service.domain.job`): replaces `MultiTurnExecutor`'s fixed-`N` loop
...
- [ ] 6.4 `EvaluationWorker.execute` dispatch: route every DEPLOYMENT HTTP case (single-turn and
  ...
  regression test).
- [ ] 6.5 Functional test: a multi-turn dataset bound to a suite with **no** per-turn binding produces
  exactly one request/result row per test case (collapse case, Decision 4), not `multiTurnData.length`
  rows.
- [ ] 6.6 Functional test: a multi-turn dataset bound to a suite **with** a per-turn binding produces `N`
...
- [ ] 7.1 `CustomContentAccumulator` (new, `service.domain.job`): merges `choices[i].delta.custom_content`
...
- [ ] 7.2 Wire `CustomContentAccumulator` into `StreamingResponseAccumulator`'s OpenAI-mode assembly so the
  final assembled body carries `choices[0].message.custom_content`.
- [ ] 7.3 Unit test: a multi-chunk stream with `custom_content` on different chunks assembles onto
...
- [ ] 8.1 Functional test suite: Map-content single-turn suite unchanged behavior (regression); String-content
  JSONata body suite (new); multi-turn suite with per-turn binding (N turns, history via frame) vs. without
  (N=1 collapse); streaming DIAL custom-content accumulation across turns; reserved-name 400s at suite save
  time; JSONata write-time 400 for invalid String-content source.
- [ ] 8.2 Sync delta specs (`multi-turn-test-case`, `request-template`, `response-columns` under this
  ...
  gained content and did not lose existing requirements.
- [ ] 8.3 Update `AGENTS.md`: replace the `multi-turn-test-case` inline convention's "Assistant reply path
...
- [ ] 8.4 Update OpenAPI examples for `RequestBodyDto` (String-content variant) and any changed
...
- [ ] 8.5 Update `openspec/specs/README.md` summaries for `multi-turn-test-case`, `request-template`,
  ...
  Maintenance Policy).
- [ ] 8.6 Run `./gradlew clean build` (full build: tests + Checkstyle + Spotless check + ArchUnit/
