...
Multi-turn conversations are an ordered group of `test_cases` rows sharing a `conversation_id`, with a `turn_index`. Today the run pipeline treats a conversation as runnable only when its surviving (valid ∧ not-disabled ∧ filter-matching) turns form a **contiguous prefix `0..k`**. Any deviation — no turn 0, a gap, a filtered-out or disabled middle turn — is classified **broken** at snapshot time and produces a single `0/0` sentinel ERROR row at execution.

The break is driven entirely by `ConversationAssembler.assemble`:
...
boolean broken = anyInvalid || !contiguous || survivors.size() > MAX_CONVERSATION_TURNS;
```

Everything downstream (`SnapshotInputWriter`, `MultiTurnConversationExecutor`, `EvaluationWorker`) consumes the assembler's output; none of them re-derive ordering. The one non-obvious coupling is `turn.last` in conditional-metric execution: `ConditionExpressionEvaluator` computes `turn.last = (turnIndex + 1) == totalTurns`, which is only correct because contiguity guarantees the last survivor's authored index equals `count - 1`.

This design lifts the ordering/sequencing constraint while keeping the ability to (a) fail a conversation loudly when a surviving turn is invalid or the turn count is excessive, and (b) evaluate `turn.last` correctly once gaps exist.
...
- Keep a loud, visible failure for a genuinely unusable conversation (invalid surviving turn, or over-cap).
...
- Making `MAX_CONVERSATION_TURNS` configurable (stays a non-configurable constant).
...
Remove `isContiguousFromZero` and the `contiguous` term from `ConversationAssembler`. Survivors are still sorted ascending by authored `turn_index` and run in that order. A missing turn 0, a gap, or a filtered-out middle turn simply yields a shorter survivor list.
...
### D2 — Invalid surviving turn still breaks the whole conversation (visible error)
An `is_valid = false` turn among the survivors keeps breaking the conversation → one `0/0` sentinel ERROR row, no model call, run continues. This is a deliberate divergence from single-turn (which silently excludes invalid rows): a half-run conversation with a silently-dropped invalid middle turn would be misleading, so we surface it.
...
Result rows keep the authored `turn_index`. Because gaps break the `(turnIndex + 1) == totalTurns` identity, introduce `lastTurnIndex` = the maximum authored index among survivors, computed once in `ConversationAssembler` and carried on `AssembledConversation`. `MultiTurnConversationExecutor` stamps it onto every `TestCaseRunResult` of the conversation. `total_turns` keeps its meaning — the count of surviving turns that ran.
...
The condition dictionary is built at metric-evaluation time in `InProcessMetricEvaluationExecutor` **from the persisted result row**, not from the snapshot-time `AssembledConversation`. A single turn row cannot know the conversation's max authored index unless it is stored. So `last_turn_index` must live on the result row. Migration `V{n}__AddLastTurnIndexToTestCaseRunResults.sql` adds `last_turn_index INTEGER NOT NULL DEFAULT 0`; it is not part of the idempotency key (which already includes `turn_index`). Regenerate jOOQ and commit.

**Alternative considered — recompute at metric-eval time by loading sibling turns:** rejected; it turns a per-row evaluation into a per-conversation load and duplicates grouping logic already owned by the assembler.
...
Remove the `turnIndex < MAX_CONVERSATION_TURNS` check from `ConversationFieldsValidator` (with gaps, a high index no longer implies many turns). Keep both-or-neither, valid UUID, `turnIndex >= 0`, and the duplicate `(conversation_id, turn_index)` 409. The cap becomes a **surviving-turn-count** guard in `ConversationAssembler` (`survivors.size() > MAX_CONVERSATION_TURNS` ⇒ broken). `MAX_CONVERSATION_TURNS` remains a constant.

### D7 — Sentinel-row wording
`EvaluationWorker.buildBrokenConversationResult` keeps producing the `0/0` ERROR row but the `BROKEN_CONVERSATION` message is reworded to only reference the two remaining causes (an invalid turn, or too many turns) — no mention of turn 0 / contiguity. The sentinel row carries `last_turn_index = 0`.
...
Snapshot tx:
  SnapshotInputWriter
    → RunnableTestCaseSelector.loadConversationTurns(filter applied in SQL)
    → ConversationAssembler.assemble(turns, excludedIds)
         survivors = filter-matching − disabled, sorted by authored turn_index
         broken = anyInvalid(survivors) || survivors.size() > MAX_CONVERSATION_TURNS
         lastTurnIndex = max authored index among survivors      ← NEW
         → AssembledConversation{ turnsJson, broken, totalTurns=count, lastTurnIndex }  ← NEW field
    → TestCaseRunInput (turns JSONB, broken)

Execution:
  EvaluationWorker.execute
    broken → buildBrokenConversationResult (0/0 ERROR, last_turn_index=0, reworded msg)
    turns  → MultiTurnConversationExecutor.execute
...
- **Invalid-turn asymmetry** (single-turn drops invalid, conversation breaks on invalid) → could surprise. Mitigation: documented in the spec and the sentinel error message; it is the explicit product decision (D2).
