...
three fields: `turn.index` (the authored 0-based turn index, preserved even when a conversation's
surviving turns are non-contiguous), `turn.total` (the conversation's surviving turn count that ran),
and `turn.last` (boolean, true when `index == lastTurnIndex`, where `lastTurnIndex` is the maximum
authored `turn_index` among the conversation's surviving turns — sourced from the result row's
...
the final turn of a conversation.
Status: **Implemented**

#### Scenario: turn.last on the final turn
- **WHEN** a condition is `turn.last` and the result is the turn whose authored `turn_index` equals the conversation's `last_turn_index` (the last surviving turn)
- **THEN** the condition SHALL evaluate to `true` and the metric SHALL run

#### Scenario: turn.last on a non-final turn
- **WHEN** a condition is `turn.last` and the result is a turn whose authored `turn_index` is less than the conversation's `last_turn_index`
...
- **WHEN** a conversation's surviving turns have authored `turn_index` `0` and `3` (so `total = 2`, `last_turn_index = 3`) and a condition is `turn.last`
