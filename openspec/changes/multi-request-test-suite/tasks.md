...
- [x] 6.4 Implement the chain executor — sequential loop over the normalized chain under one permit, accumulating extracted response columns, resolving `responseField` from that map, persisting one row per request with `request_index`/`request_label` and `turn_index=0`/`total_turns=1` (done: a 3-request chain writes 3 rows and a later request consumes an earlier request's value; `nullExtractedValueDoesNotBreakAccumulation` covers a column that matched nothing accumulating as an explicit null, and `notIssuedStepUnderCancellationWritesNoRow` the un-issued-step row rule)
