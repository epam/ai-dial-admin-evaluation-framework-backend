...
- **Execution context plumbing**: `service.domain.job.EvaluationContext` (+`snapshotMultiStep` boolean, +`snapshotMultistepInputBindings`); `TestSuiteEvaluationJob.buildContext` populates both from the resolved `SuiteSnapshotDto`. `EvaluationWorker` reads discrete `context.getSnapshotX()` getters (it never accesses `SuiteSnapshotDto` directly), so the multi-step flag and per-step bindings must travel through `EvaluationContext`.
...
- **Metrics**: a new shared injectable `service.domain.job.ExtractedColumnsNormalizer` (array→last-element; empty array→`{}`; object→as-is) applied at **two** call sites — `MetricEvaluationWorker.buildRequest` (the `extractedColumns` value passed to `BindingResolver.resolveBindings`, scoped so `testCaseData` is untouched) and `InProcessMetricEvaluationExecutor.buildItem` (the private `parseJsonNode(result.getExtractedColumns())` copy into `EvalSummary`). The downstream summary/export/query layer (`EvalSummaryExportRow`, `EvalSummaryExportColumnPlanner`, `JsonbFieldResolver`) is **unaffected** because `EvalSummary` keeps an object shape.
