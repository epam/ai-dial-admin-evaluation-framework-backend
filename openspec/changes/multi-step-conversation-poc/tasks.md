...
- [ ] 4.1 Create injectable `service.domain.job.MultiStepConversationExecutor` implementing the turn loop: per step resolve template (`ResolvedRequestService.resolve`), append resolved `messages` to running history, overwrite body `messages` with full history, invoke non-streaming with per-step `invokeWithRetries`, extract `choices[0].message.content` → append assistant message, run `ResponseColumnExtractor.extract` and accumulate per-step array (done: returns one `TestCaseRunResult` with `responseBody`=accumulated messages, `extractedColumns`=per-step array).
- [ ] 4.2 Implement fail-fast in the executor: stop on a step's retry-exhausted failure or unextractable assistant content; persist partial history + partial per-step extractions; set `executionStatus`/`responseStatusCode` from the failing step; `traceId` = last attempted step (done: matches fail-fast scenarios).
- [ ] 4.3 Branch `EvaluationWorker.execute` on snapshot `multiStep`: delegate to `MultiStepConversationExecutor` when true, keep existing single path when false; one semaphore permit per conversation (no change to `InProcessEvaluationExecutor` permit granularity) (done: single-step path unchanged; multi-step delegates).

## 5. Metric normalization

- [ ] 5.1 Add shape-detection normalization at the result→metric boundary: in `BindingResolver.parseJsonMap` (or a shared helper it calls), if `extractedColumns` JSON is an array use the last element, else use the object as-is (done: metric bindings resolve against the last step for multi-step; object unchanged for single-step).
- [ ] 5.2 Ensure `InProcessMetricEvaluationExecutor` copies the normalized (last-step) object into `EvalSummary.extractedColumns` so the summary/export/query layer stays object-shaped (done: `EvalSummary` never stores an array; export/filter paths unchanged).
...
- [ ] 7.3 Unit tests for the metric shape-detection normalization (array→last element; object→unchanged) (done: covers length-1 array and object).
