...
- **WHEN** a caller (e.g. the Query DSL's `MetricScoreLatestComputationDefaulter`) does not have the run's `created_at_ms` available
...
- Migration: `src/main/resources/db/migration/analytics/POSTGRES/V1.20__PartitionAnalyticsHighVolumeTables.sql` — ATTACH-based cutover with a `_p_legacy` partition for all three tables, computed from one shared `now()`-derived cutover at migration time; `test_case_eval_scores` additionally gets a new `created_at_ms` column backfilled via `UPDATE ... FROM test_case_eval_summaries` and a new composite PK `(created_at_ms, eval_summary_id)` before the ATTACH step. See `design.md` D3.
- New components: `AnalyticsPartitionMaintenanceJob`, `AnalyticsPartitionMaintenanceService`, `MonthlyPartitionBounds` (`service.infrastructure.partition`); `AnalyticsPartitionRepository` / `PostgresAnalyticsPartitionRepository` (`data.db.analytics.repository`); `AnalyticsPartitioningProperties` (`configuration.properties.analytics`); `AnalyticsPartitioningConstants` (`constants`).
