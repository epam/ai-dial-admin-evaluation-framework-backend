...
Planned; no code paths yet. Design and rationale: `openspec/changes/run-comparison-metric-scores/design.md`.
The recomputation reuses the existing Phase-3 query definitions (`BuiltInMetricStatistics`,
`OverallScoreDefinitionResolver` in `experimental/query/service/metricscore/`) unchanged, adding only a
row-exclusion predicate, which is what makes the full-overlap parity scenario hold structurally.
