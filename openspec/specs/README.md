...
- **[multi-turn](multi-turn/spec.md)** — Implemented
  Row-based multi-turn evaluation — a multi-turn is an **ordered group of `test_cases` rows** (one row per turn) sharing a top-level `multiTurnId` (client-supplied UUID) with contiguous `turnIndex` from 0; both NULL ⇒ single-turn. Multi-turn is **emergent from the data**, with NO suite-level `multiTurn` flag and no array-valued columns. Authoring is client-managed (per-row write validation: both-or-neither, UUID, `turnIndex` in `[0, MAX_MULTI_TURN_TURNS)`, duplicate `(multiTurnId, turnIndex)` → 409); contiguity/completeness is validated ONCE at snapshot. Counting is per **individual test case** (multi-turn turns counted individually — no per-multi-turn grouping in the guard/count path); the `testCaseFilter` is applied **row-level, like disable**. Multi-turn grouping + integrity (all turns valid, enabled turns a contiguous prefix `0..k`, tail-only truncation, middle hole ⇒ broken) is resolved **only at snapshot**. Snapshot pages by distinct `multiTurnId`, assembles each multi-turn into ONE `test_case_run_inputs` unit (ordered frozen turns + `broken` flag), and overwrites `number_of_test_cases` with the execution-unit count (multi-turn = 1 unit); execution dispatches one worker task per unit. `MultiTurnExecutor` iterates the frozen turns (each turn's own scalar `data`), full-history resend, assistant reply appended as the full `choices[0].message` verbatim (non-streaming only), fail-fast on turn failure. **Each turn produces its own scalar `TestCaseRunResult` row** keyed by `turn_index`/`total_turns` (single-turn rows read `0/1`); a broken multi-turn yields a single `0/0` sentinel ERROR row without a model call and the run continues. HTTP-deployment only — an MCP suite bound to a dataset with multi-turn rows is rejected at run creation (409). Metrics evaluate against each turn's raw scalar columns. Owned by `MultiTurnExecutor`/`MultiTurnAssembler` (delegated from `EvaluationWorker`, which returns `List<TestCaseRunResult>`). Related: eval-execution-engine, test-cases, suite-run-snapshot, test-suites, metric-evaluation.
