...
- **[metric-score-statistics](metric-score-statistics/spec.md)** — Implemented
  Seed-only Query-DSL metric-score definitions (`DEFAULT` predefined stats: AVG, P10, P90, MIN, MAX) plus a DSL-computed `overall` (unweighted mean of per-metric averages via `mean(:metricAvgs)`), computed automatically at run completion (Phase 3) into `metric_score_result` over the run's eval summaries. Definitions are seed-only (no management API); a results read API is exposed (`/api/v1/analytics/metric-score-results`). Dispatch is param-driven (no `kind` column); the executor lives in `experimental.query.service.metricscore` behind a `service`-layer `MetricScoreComputation` interface. Relies on the structured-query-model `param` substitution and `mean` function. Related: structured-query-model, metric-evaluation, metrics-storage.
