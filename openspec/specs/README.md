...
- **[conditional-metric-execution](conditional-metric-execution/spec.md)** — Implemented
  Optional per-metric execution `condition` on a TSMD deciding, per test-case result, whether that metric runs. Evaluated against a namespaced `{data, response}` dictionary (explicit-null-preserving serialization); a bare `name()` is a registered custom function (SPI `ConditionFunction` + registry, shipped empty), anything else is JSONata. Validated eagerly at write time (hard 400, no soft `is_valid` impact); at run time a clean `true` runs, a clean `false` omits the metric entirely, and any other outcome records a wholesale `metricError::<name>` without changing the test-case `executionStatus`. Related: test-suite-metric-definitions, metric-evaluation, eval-summary-export.
