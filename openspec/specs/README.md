...
- **[multi-turn-conversation](multi-turn-conversation/spec.md)** — Implemented
  Multi-turn conversation evaluation POC — the `multiTurn` suite flag drives a chat-completions conversation reusing the suite's single `inputBindings` and `requestTemplate` per turn. Per-turn variation comes from array-valued bound columns in the test-case data; the turn count `N` is derived PER TEST CASE from the common length of those arrays (scalar columns and `constantValue` bindings broadcast on every turn, capped at `MAX_CONVERSATION_TURNS`), so different test cases may run different numbers of turns. Per-turn loop with full-history resend, assistant reply appended as the full `choices[0].message` object verbatim (non-streaming only), fail-fast on turn failure. Test-case-level data problems (bad array shape, no array column, over-cap) fail only that test case with `ERROR`. **Each turn produces its own scalar `TestCaseRunResult` row** keyed by `turn_index`/`total_turns` (single-turn rows read `0/1`); a turn's `extractedColumns` is a plain object of scalars and `responseBody`/`requestBody` hold that turn's raw response/accumulated request. Abort persists `k` SUCCESS rows + 1 ERROR row (`total_turns=N`); a data-shape failure yields a single ERROR row with the `0/0` sentinel. Metrics evaluate against each turn's raw scalar columns (no JSONata/array machinery). Owned by `MultiTurnConversationExecutor` (delegated from `EvaluationWorker`, which now returns `List<TestCaseRunResult>`). Related: eval-execution-engine, test-suites, metric-evaluation.
