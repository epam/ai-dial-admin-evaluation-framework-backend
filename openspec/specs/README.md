...
- **[suite-test-case-filter](suite-test-case-filter/spec.md)** — Implemented
  Per-suite `testCaseFilter` — an optional Structured Query DSL filter over the bound dataset's test-case fields that narrows the runnable subset. AND-combined with `is_valid` only — it is the single mechanism for narrowing a suite's runnable subset; applied consistently at run-creation's zero-runnable guard (409 `INVALID_OPERATION` when no match) and at snapshot materialization. Reuses the Structured Query DSL translation via the `service.domain.job.RunnableTestCaseSelector` interface inversion (no `service` → `experimental.query` dependency). Related: test-suites, query-schema-discovery, structured-query-model, suite-run-snapshot, test-suite-runs.
