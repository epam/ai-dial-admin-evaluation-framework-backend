...
- **[multi-turn-conversation](multi-turn-conversation/spec.md)** — Implemented
  Multi-turn test cases as a single test case carrying an ordered `multiTurnData` array of per-turn data maps, mutually exclusive with single-turn `data` (DB CHECK). Per-turn schema validation, configurable turn-count cap (`test-case.multi-turn.max-turns`, default 10), sequential turn-loop execution with full-history resend and fail-fast, flat CSV multiplication (reserved `turnIndex` column, contiguous-run assembly on import), and MCP-suite rejection at run creation. Related: test-cases, eval-execution-engine, suite-run-snapshot, suite-test-case-filter, conditional-metric-execution.
