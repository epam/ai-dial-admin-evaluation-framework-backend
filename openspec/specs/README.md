...
- **[dataset-clone](dataset-clone/spec.md)** — Implemented
  Standalone dataset clone endpoint (`POST /api/v1/datasets/{id}/clone`) — deep-copies a dataset (row + all test cases with fresh IDs and `@ef/datasets/{id}/` file-ref rewrites) into a new dataset that inherits the source's visibility and is unbound to any suite. Any source (PUBLIC or PRIVATE) may be cloned; the source is never modified. Optional `name` (auto-derived `"<source> (clone)"` when omitted) and `description` (copied from source when omitted) overrides; 404 when source missing, 409 on duplicate name. Reuses `DatasetCloneService.cloneRowAndTestCases` (visibility/description threaded as parameters). Related: datasets, test-suite-clone, detach-dataset.
