...
- **[suite-pass-rate](suite-pass-rate/spec.md)** — Implemented
  `GET /api/v1/analytics/eval-summaries/test-case-pass-rate/{testSuiteId}?lastN=` — per-run test-case pass-rate breakdown over a suite's most recent `lastN` runs of any status (default `analytics.pass-rate.default-last-n`, capped at `analytics.pass-rate.max-last-n`; above the cap is HTTP 400). Each run buckets its eval-summary rows into `failedCount` (execution_status <> SUCCESS), `successPassedCount` (SUCCESS ∧ passed IS TRUE), `successNotPassedCount` (SUCCESS ∧ passed IS FALSE), and `successNoVerdictCount` (SUCCESS ∧ passed IS NULL — no score row or no threshold), summing to `totalCount`; runs with no eval summaries are omitted, RUNNING runs included with partial counts. Buckets are computed against each run's **latest computation** (max `computed_at_ms`, ties broken by `computation_id DESC` under text ordering). Related: metrics-storage, eval-summary-scoring, run-comparison-metric-scores.
