...
- **[test-suites](test-suites/spec.md)** — Implemented
  TestSuite CRUD with suite type discriminator (DEPLOYMENT, MCP_TOOL). DEPLOYMENT suites: deploymentRef, endpointRef, requestTemplate, inputBindings. MCP_TOOL suites: mcpDeploymentRef, toolRef, argumentTemplate. Suite binds to a dataset via optional `datasetId` (suites with `datasetId=null` are "unbound" — retrievable and updatable but cannot run). Rebinding off a PRIVATE dataset is forbidden (409 PRIVATE_DATASET_REBIND_FORBIDDEN). Suite validity (`isValid`) is config-only (template + bindings + endpoint schema; for MCP_TOOL suites also required-argument coverage against `toolRef.inputSchema`); a bound suite with zero runnable test cases is `isValid=true`. Run creation enforces a run-time guard: zero runnable test cases → 409 `INVALID_OPERATION`. Optional `testCaseFilter` further narrows the runnable subset (see `suite-test-case-filter`). Related: datasets, request-template, test-cases, mcp-tool-invocation, suite-test-case-filter.
- **[multi-request-suite](multi-request-suite/spec.md)** — Implemented
  A suite may execute an ordered **request chain** per test-case repetition against its single `deploymentRef`: the suite's own request fields are request #0, and `additionalRequests` (`List<RequestDefinitionDto>`, capped at 10) supplies requests 1..N, run strictly sequentially. Response columns across the whole chain form one flat, globally-unique namespace (suite-wide 50-column cap); the JSONata frame accumulates across requests and turns, so a later request's template can reference an earlier request's extracted column by name, and each persisted row's `extracted_columns` is the accumulated union at that point. Turn count is decided per request (any subset of the chain may be multi-turn). Every chain call persists its own `test_case_run_results`/`test_case_eval_summaries` row, carrying the new `(request_index, total_requests)` dimension alongside the existing turn dimension; metrics run on all rows by default, scoped per-request via a TSMD `condition`'s new `request: {index, total, last, name}` namespace. Fail-fast aborts the rest of the chain on the first failing call. `additionalRequests` non-empty on an `MCP_TOOL` suite is rejected (400) — MCP chaining is deferred. Orchestrated by `runner.job.RequestChainExecutor`. Related: test-suites, request-template, response-columns, multi-turn-test-case, suite-run-snapshot, conditional-metric-execution, analytics-eval-results, metrics-storage, eval-summary-export, run-comparison-metric-scores, query-schema-discovery, tsmd-validation.
- **[suite-test-case-filter](suite-test-case-filter/spec.md)** — Implemented
  Per-suite `testCaseFilter` — an optional Structured Query DSL filter over the bound dataset's test-case fields that narrows the runnable subset. AND-combined with `is_valid` only — it is the single mechanism for narrowing a suite's runnable subset; applied consistently at run-creation's zero-runnable guard (409 `INVALID_OPERATION` when no match) and at snapshot materialization. Reuses the Structured Query DSL translation via `query.service.QueryDslRunnableTestCaseSelector`, injected directly into the run/suite services. Related: test-suites, query-schema-discovery, structured-query-model, suite-run-snapshot, test-suite-runs.
- **[test-suite-clone](test-suite-clone/spec.md)** — Implemented
  Clone endpoint (`POST /api/v1/test-suites/{id}/clone`) — deep-copies a test suite and its metric definitions (TSMDs) under a new name; cloned suite references the source's dataset by default (test cases are NOT duplicated — they belong to the dataset). Exception: cloning a suite bound to a PRIVATE dataset (no `datasetId` override) also clones the dataset — a new PRIVATE dataset with copied test cases (new ids) and copied dataset-scoped files (no suite-level id remapping — the clone rewrites only `dataset_id`); redirecting such a clone to a different dataset is forbidden (409 PRIVATE_DATASET_REBIND_FORBIDDEN). Optional overrides: description, deploymentRef, endpointRef, responseColumns, requestTemplate, inputBindings, mcpDeploymentRef, toolRef, argumentTemplate, datasetId. suiteType is always inherited. File references in suite/TSMD JSONB rewritten to the new suite scope. Hard write-time validation (the create/update rule set) runs against the effective post-override suite and rejects with 400 before anything is created; the soft `isValid` / `validationWarnings` determination that follows is synchronous only.
- **[detach-dataset](detach-dataset/spec.md)** — Implemented
  Detach-dataset endpoint (`POST /api/v1/test-suites/{id}/detach-dataset`) — forks a suite's bound PUBLIC dataset into a new PRIVATE clone for exclusive use by that suite; the original PUBLIC dataset is preserved. Test cases are copied with fresh IDs and file-ref rewrites; the suite rebind touches `dataset_id` only. Optional `name` field in the request body; derived automatically when omitted. 409 when suite has no dataset (SUITE_HAS_NO_DATASET) or bound dataset is already PRIVATE (PRIVATE_DATASET_REBIND_FORBIDDEN).
...
- **[test-suite-run-costs](test-suite-run-costs/spec.md)** — Implemented
  `GET /api/v1/costs/test-suite-run/{id}` (canonical) and `GET /api/v1/test-suite-runs/{id}/costs` (backward-compatible alias) — average cost per test case for a run's execution phase and its metric-evaluation phase, computed from a dial-adas raw-SQL query (`execute-sql`) that sums `total_price` per `testcase.id` baggage tag before averaging across test cases, correlated by the run's OTel baggage (`eval.run.id`, `eval.phase`, `testcase.id`). `CostController`, `DialAdasClient` (`executeAggregate` for structured queries, `executeSql` for raw SQL), `AdasCostQueryBuilder` (`buildAvgCostPerTestCaseSql` raw SQL; `buildDeploymentAggregateQuery`/`buildPageTotalCostQuery` still built as a real `StructuredQuery` — dial-adas shares the same query DSL grammar). Related: test-suite-runs, grafana-deep-links, observability-and-logging, structured-query-model, deployment-costs.
...
- **[test-suite-runs-query-entity](test-suite-runs-query-entity/spec.md)** — Implemented
  `test_suite_runs` as a structured-query entity — derived-table projection excluding heavy JSONB, snapshot-only deployment/MCP refs, latest-computation `metric_names`. Related: structured-query-model, query-schema-discovery, metrics-storage, test-suite-runs.
- **[query-result-page-extension](query-result-page-extension/spec.md)** — Implemented
  Post-execution seam letting a structured-query result page carry values its own SQL cannot produce — typically from another datasource. A `QueryResultPageExtender` SPI plus the `@Primary` coordinator that folds a page through every registered extender in bean order, each in its own try/catch so one misbehaving extender (throwing, or returning null) never fails a query that succeeded. Establishes the **open key set**: a row's keys are the projection's plus zero or more extension-derived keys, which stay unqueryable and unpublished, so schema discovery keeps advertising exactly the queryable field set. First implementation attaches `overall_score_value` to `test_suite_runs` rows. Related: structured-query-model, query-schema-discovery, test-suite-runs-query-entity, metric-score-statistics.
...
- **[metrics-storage](metrics-storage/spec.md)** — Implemented
  Eval summary storage layer — denormalized analytics table for metric-enriched test case results, computation versioning, JSONB metric filtering and aggregation. `metric_eval_duration_ms` column (required on batch write, exposed on response DTOs) tracks the summed per-TSMD metric evaluation latency. Run metric snapshots are stored separately, in the **meta** database (`run_metric_snapshots`, FK to `test_suite_runs` with `ON DELETE CASCADE`), and served by `/api/v1/run-metric-snapshots` with `/api/v1/analytics/run-metric-snapshots` kept as a deprecated alias; the same-named analytics table is frozen and read by nothing.
...
- **[database-and-migrations](database-and-migrations/spec.md)** — Implemented
  PostgreSQL JDBC + Flyway migration conventions. Migrations may be SQL or Java, with all schema DDL in SQL and Java migrations restricted to data movement and registered explicitly rather than classpath-scanned; the meta Flyway instance runs after the analytics one, so a meta migration may read a fully migrated analytics database.
...
- **[test-suite-runs](test-suite-runs/spec.md)** — Implemented
  Foundational infrastructure for async test suite execution: run lifecycle (PENDING→RUNNING→(CANCELLING→)COMPLETED/FAILED/CANCELLED), CRUD with filtering/sorting/pagination, SSE status streaming, startup reconciliation, configurable concurrency limits. Uses in-process evaluation engine on thread-per-task executors (virtual by default, platform opt-out via `spring.threads.virtual.enabled`); run concurrency governed solely by `test-suite-run.limits.*`; streaming SSE support, retry/rate-limiting, and configurable execution settings. `numberOfTestCases` finalized at snapshot phase (not immutable at creation). `suiteSnapshot` field in API response (detail only; omitted from list).
