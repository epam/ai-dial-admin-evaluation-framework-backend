...
- **[multi-step-conversation](multi-step-conversation/spec.md)** — Implemented
  Multi-turn (multi-step) conversation evaluation POC — the `multiStep` suite flag drives a chat-completions conversation reusing the suite's single `inputBindings` and `requestTemplate` per turn. Per-turn variation comes from array-valued bound columns in the test-case data; the turn count `N` is derived PER TEST CASE from the common length of those arrays (scalar columns and `constantValue` bindings broadcast on every turn, capped at `MAX_CONVERSATION_STEPS`), so different test cases may run different numbers of turns. Per-step turn loop with full-history resend, assistant reply appended as the full `choices[0].message` object verbatim (non-streaming only), fail-fast on step failure. Test-case-level data problems (bad array shape, no array column, over-cap) fail only that test case with `ERROR`. Reuses existing result columns (`responseBody` = the last turn's raw response body, mirroring `requestBody`; `extractedColumns` = a column-major object of per-column arrays, one element per turn). Metrics read the raw `extractedColumns` (no normalization) and select a specific turn/element via a `Response`/`TestCase` binding's optional `jsonataExpression`. Owned by `MultiStepConversationExecutor` (delegated from `EvaluationWorker`). Related: eval-execution-engine, test-suites, metric-evaluation.
