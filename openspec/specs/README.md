...
- **[test-suite-metric-definitions](test-suite-metric-definitions/spec.md)** — Implemented
  TSMD CRUD — materialized metric configurations within a test suite, polymorphic parameter bindings (TestCase, Response, Constant), client-supplied metric declaration version with ownership validation, `enabled` flag, optional execution `condition` (validated eagerly, hard 400 on malformed — see `conditional-metric-execution`), `valid` / `validationWarnings` state. Related: test-suites, metrics-system, tsmd-validation, conditional-metric-execution.
...
- **[multi-turn-conversation](multi-turn-conversation/spec.md)** — Implemented
  Row-based multi-turn conversation evaluation — a conversation is an **ordered group of `test_cases` rows** (one row per turn) sharing a top-level `conversationId` (client-supplied UUID) with contiguous `turnIndex` from 0; both NULL ⇒ single-turn. Multi-turn is **emergent from the data**, with NO suite-level `multiTurn` flag and no array-valued columns. Authoring is client-managed (per-row write validation: both-or-neither, UUID, `turnIndex` in `[0, MAX_CONVERSATION_TURNS)`, duplicate `(conversationId, turnIndex)` → 409); contiguity/completeness is validated ONCE at snapshot. Counting is per **individual test case** (conversation turns counted individually — no per-conversation grouping in the guard/count path); the `testCaseFilter` is applied **row-level, like disable**. Conversation grouping + integrity (all turns valid, enabled turns a contiguous prefix `0..k`, tail-only truncation, middle hole ⇒ broken) is resolved **only at snapshot**. Snapshot pages by distinct `conversationId`, assembles each conversation into ONE `test_case_run_inputs` unit (ordered frozen turns + `broken` flag), and overwrites `number_of_test_cases` with the execution-unit count (conversation = 1 unit); execution dispatches one worker task per unit. `MultiTurnConversationExecutor` iterates the frozen turns (each turn's own scalar `data`), full-history resend, assistant reply appended as the full `choices[0].message` verbatim (non-streaming only), fail-fast on turn failure. **Each turn produces its own scalar `TestCaseRunResult` row** keyed by `turn_index`/`total_turns` (single-turn rows read `0/1`); a broken conversation yields a single `0/0` sentinel ERROR row without a model call and the run continues. HTTP-deployment only — an MCP suite bound to a dataset with conversation rows is rejected at run creation (409). Metrics evaluate against each turn's raw scalar columns. Owned by `MultiTurnConversationExecutor`/`ConversationAssembler` (delegated from `EvaluationWorker`, which returns `List<TestCaseRunResult>`). Related: eval-execution-engine, test-cases, suite-run-snapshot, test-suites, metric-evaluation.
...
- **[metric-evaluation](metric-evaluation/spec.md)** — Implemented
  In-process metric evaluation engine — Phase 2 of test suite run lifecycle. Evaluates configured TSMDs against test case results by calling metric provider `/evaluate` endpoints with resolved bindings, writes results as EvalSummary records. Provider-bounded concurrency, retry with exponential backoff, RunMetricSnapshot capture. Honors each TSMD's optional execution `condition` (see `conditional-metric-execution`): before dispatch, a false condition omits the metric and a bad condition records a `metricError` without failing the summary; the `TsmdEvaluationResult` carrier has three variants (`Success`/`Failure`/`ConditionError`). Related: conditional-metric-execution, test-suite-metric-definitions, metrics-storage.
- **[conditional-metric-execution](conditional-metric-execution/spec.md)** — Implemented
  Optional per-metric execution `condition` on a TSMD deciding, per test-case result (per turn), whether that metric runs. Evaluated as a JSONata expression against a namespaced `{data, response, turn}` dictionary (explicit-null-preserving serialization); the `turn` namespace exposes `index`/`total`/`last` so a condition can gate on turn position (e.g. `turn.last` runs only on the final turn). Validated eagerly at write time (hard 400 on malformed JSONata, no soft `is_valid` impact); at run time a clean `true` runs, a clean `false` omits the metric entirely, and any other outcome records a wholesale `metricError::<name>` without changing the test-case `executionStatus`. Related: test-suite-metric-definitions, metric-evaluation, eval-summary-export.
- **[metric-score-statistics](metric-score-statistics/spec.md)** — Implemented
  Code-defined per-metric aggregate statistics (AVG/P10/P90/MIN/MAX) computed as Phase-3 of a test suite run via the structured-query DSL, stored in `metric_score_results` analytics table, readable only through the unified Query DSL entity `metric_score_results`. Per-suite `overall_score` column and snapshot field reserved as a future extension point; default `overall` is the single metric's average for single-metric runs. No management API; no dedicated read endpoint.
