...
- **[multi-step-conversation](multi-step-conversation/spec.md)** — Implemented
  Multi-turn (multi-step) conversation evaluation POC — `multiStep` + `multistepInputBindings` suite config drives a fixed, author-scripted chat-completions conversation: per-step turn loop with full-history resend, hardcoded `choices[0].message.content` assistant extraction (non-streaming only), fail-fast on step failure. Reuses existing result columns (`responseBody` = the last turn's raw response body, mirroring `requestBody`; `extractedColumns` = per-step array); metrics normalize the array to its last element via `ExtractedColumnsNormalizer`. Owned by `MultiStepConversationExecutor` (delegated from `EvaluationWorker`). Related: eval-execution-engine, test-suites, metric-evaluation.
