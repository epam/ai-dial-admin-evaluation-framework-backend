...
`condition` string on a metric that decides, per test-case result, whether that metric runs. The
condition is either a JSONata expression or a registered custom-function call, evaluated against a
namespaced `{data, response}` dictionary.
