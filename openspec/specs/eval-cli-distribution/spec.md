...
This spec defines the distribution of the `eval-cli` standalone CLI as a Docker image: how the image is built, how arguments are passed through to the underlying jar, and how the image is automatically published to GHCR without interfering with the main application's own deploy pipeline.
