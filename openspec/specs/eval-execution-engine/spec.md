...
The `InProcessEvaluationExecutor` SHALL read test inputs from the `test_case_run_inputs` table (populated at snapshot phase) in pages, dispatch execution tasks (**one per multi-turn per run index**) bounded by the configured concurrency level, collect results, and flush them to analytics DB in batches. For legacy runs without a snapshot (no `test_case_run_inputs` rows), it SHALL fall back to reading live test cases from the suite, treating each live test case as a length-1 (single-turn) multi-turn. The **execution unit is a MULTI_TURN**: one assembled `test_case_run_inputs` row holds the ordered turns of a multi-turn (a single-turn test case is a length-1 multi-turn), and one such row maps to exactly one worker task per run index.
Status: **Implemented**
...
- **THEN** the executor SHALL fall back to paging through live test cases from `testCaseRepository.findEnabledValidByTestSuiteId()`, wrapping each `TestCase` as a length-1 multi-turn `TestCaseRunInput` struct for a uniform worker interface.

#### Scenario: Sequential execution (default)
- **WHEN** `concurrencyLevel` is 1 (default)
- **THEN** the executor SHALL process multi-turns one at a time, in order (page by page, multi-turn by multi-turn, run index by run index); the turns within a multi-turn always run sequentially regardless of `concurrencyLevel`.

#### Scenario: Parallel execution
- **WHEN** `concurrencyLevel` is greater than 1 (e.g., 10)
- **THEN** the executor SHALL process up to `concurrencyLevel` multi-turn tasks concurrently using a semaphore-bounded virtual thread executor (one permit per multi-turn, not per turn).

#### Scenario: One assembled input is one worker task
- **WHEN** the executor dispatches work for the run
- **THEN** it SHALL dispatch exactly one worker task per `(multi-turn × runIndex)`; the worker task owns the whole multi-turn and emits one `TestCaseRunResult` per surviving turn.

#### Scenario: All runnable multi-turns are executed
- **WHEN** the executor runs for a suite with N runnable multi-turns and `numberOfRuns = M`
- **THEN** the executor SHALL dispatch exactly N * M evaluation tasks (one per multi-turn per run index 0..M-1)

#### Scenario: Excluded turns and multi-turns are skipped
- **WHEN** the bound dataset contains test cases with `enabled = false` or `isValid = false`, or multi-turns excluded/truncated by disable/filter rules
...
- **WHEN** the suite has more runnable multi-turns than fit in a single page
- **THEN** the executor SHALL read inputs using paginated queries (a page boundary never straddles a multi-turn)

### Requirement: Single test case evaluation (worker)
The `EvaluationWorker` SHALL accept a `TestCaseRunInput` (a **multi-turn** carrying one or more ordered, frozen turns plus optional overrides) plus an `EvaluationContext` (carrying snapshot fields), resolve the request(s) without DB reads, invoke the target, extract response columns, and return a **`List<TestCaseRunResult>`** — **one element per surviving turn**. A single-turn multi-turn (one turn) SHALL return a one-element list. A multi-turn SHALL delegate turn iteration to `MultiTurnExecutor` and return one result per turn, per the multi-turn spec.
Status: **Implemented**

#### Scenario: Single-turn worker returns one result
- **WHEN** a length-1 (single-turn) multi-turn is dispatched
...
- **WHEN** a multi-turn with `N` ordered turns is dispatched
- **THEN** the worker SHALL delegate to `MultiTurnExecutor` and return `N` results (fewer on early abort — see the multi-turn fail-fast requirement), each carrying its `turnIndex` (authored, contiguous 0-based prefix) and `totalTurns` (surviving turn count)

#### Scenario: Snapshot-based request resolution
- **WHEN** a multi-turn turn is resolved for execution
...
The executor SHALL buffer completed `TestCaseRunResult` records and flush them to the analytics database in configurable row batches, performing exactly one final flush after all virtual threads terminate. Results SHALL be added per multi-turn via `addResults(buffer, List<TestCaseRunResult>)`: each call buffers all of one multi-turn's turn rows and increments a **completed-multi-turn** counter by one (a single-turn multi-turn is a one-element list; a broken multi-turn contributes a one-element ERROR list). Progress SHALL be reported as `notifyProgress(multiTurnsCompleted, totalCases)` where `totalCases = numberOfTestCases * numberOfRuns` and `numberOfTestCases` is the post-snapshot **execution-unit** count (multi-turn = 1 unit), keeping progress in the 0–100% range even though a multi-turn contributes multiple rows.
Status: **Implemented**
...
#### Scenario: Multi-turn-granular progress
- **WHEN** a 3-turn multi-turn completes in a run of 5 multi-turns
...
- **WHEN** a run of 5 standalone single-turn multi-turns (1 run each) completes
- **THEN** progress SHALL advance `1/5 … 5/5`, identical to prior behavior

#### Scenario: Broken multi-turn advances progress by one
- **WHEN** a broken multi-turn is processed and yields a single sentinel ERROR row
- **THEN** the completed-multi-turn counter SHALL advance by exactly one, the same as any other multi-turn

#### Scenario: Final flush on completion
- **WHEN** all multi-turns have been executed (run completes)
...
Guard-time counting SHALL be per **individual test case (row)**, NOT per multi-turn. `RunnableTestCaseCounter.countRunnable(...)` SHALL count individual runnable rows (`is_valid = true`, NOT in `disabledTestCaseIds`, matching `testCaseFilter`), counting each multi-turn turn as one row; there SHALL be no per-multi-turn grouping in the counting path. This row count drives the zero-runnable guard (guard #4) in `TestSuiteRunService.createRun` and coarsely seeds `number_of_test_cases` on the PENDING run. The snapshot phase SHALL then **overwrite** `number_of_test_cases` with the authoritative **execution-unit** count (standalone single-turn rows + assembled multi-turns, each multi-turn counting as exactly one unit, including broken multi-turns). Multi-turn integrity (contiguity / broken classification) is resolved ONLY at snapshot, never in the counting path.
Status: **Implemented**

#### Scenario: Guard-time runnable count is per individual test case
- **WHEN** a bound dataset has 2 multi-turns (3 turns and 4 turns) and 5 standalone single-turn rows, all runnable
- **THEN** `RunnableTestCaseCounter.countRunnable(...)` SHALL return 12 (7 turn rows + 5 single-turn rows), counting turns individually
- **AND** the PENDING run's `number_of_test_cases` SHALL be seeded to 12, then overwritten at snapshot with 7 execution units (2 multi-turns + 5 single-turn rows)
...
### Requirement: Broken multi-turn yields one sentinel ERROR row
A multi-turn that the snapshot phase marked **broken** (see the suite-run-snapshot spec for the classification rules) is frozen into a marker `test_case_run_inputs` row. At execution the worker SHALL turn that marker into **exactly ONE** `TestCaseRunResult` with `executionStatus = ERROR` and the sentinel `turn_index = 0, total_turns = 0`, **without invoking the model** (no HTTP/MCP call). The run SHALL continue; other multi-turns proceed normally.
Status: **Implemented**

#### Scenario: Broken multi-turn produces one ERROR row, no model call
- **WHEN** the worker receives an assembled input flagged as a broken multi-turn
- **THEN** the worker SHALL return a one-element `List<TestCaseRunResult>` with `executionStatus = ERROR`, `turnIndex = 0`, `totalTurns = 0`
- **AND** the worker SHALL NOT resolve or send any deployment/MCP request for that multi-turn

#### Scenario: Run continues past a broken multi-turn
- **WHEN** a run contains a broken multi-turn alongside runnable ones
- **THEN** the broken multi-turn's single ERROR row SHALL be persisted and the run SHALL still reach `COMPLETED` (the broken row does not mark the whole run FAILED)

### Requirement: One concurrency permit per multi-turn
A multi-turn SHALL execute within a single worker task holding a single concurrency permit for the entire multi-turn; its turns SHALL run sequentially within that task. Per-turn call pacing relies on the existing per-call retry and upstream 429 handling. Each turn's retries SHALL reuse the existing retry policy independently.
Status: **Implemented**

#### Scenario: Multi-turn holds one permit
- **WHEN** a multi-turn with multiple turns executes
- **THEN** the engine SHALL acquire exactly one concurrency permit for the whole multi-turn
...
- Multi-turn: injectable `service.domain.job.MultiTurnExecutor`; branch added in `EvaluationWorker.execute` keyed on the assembled input carrying more than one turn (multi-turn is emergent from multi-turn rows). The suite's single `inputBindings` are reused each turn and per-turn variation comes from each turn's discrete scalar row `data`. The worker returns a `List<TestCaseRunResult>`, one element per surviving turn; a broken multi-turn returns a one-element sentinel ERROR row (`turn_index=0, total_turns=0`) without a model call. Dispatch granularity is `(multi-turn × runIndex)` in `InProcessEvaluationExecutor` — one concurrency permit per multi-turn task; turns within a multi-turn run sequentially. See the multi-turn spec.
- Counting / guards: `RunnableTestCaseCounter.countRunnable(...)` counts individual runnable test-case rows (multi-turn turns counted individually — no per-multi-turn grouping), driving guard #4 and coarsely seeding `number_of_test_cases`; the snapshot overwrites `number_of_test_cases` with the execution-unit count (multi-turn = 1 unit). `TestSuiteRunService.createRun` keeps guard order (1 not-found, 2 unbound, 3 config-invalid, 4 zero-runnable, 5 rate-limits) and adds a guard rejecting `MCP_TOOL` suites bound to datasets containing any `multi_turn_id` row with 409 `INVALID_OPERATION`.
- Batch writer: `ResultBatchWriter.addResults(buffer, List<TestCaseRunResult>)` buffers a multi-turn's rows and advances the completed-unit counter by one; progress denominator `totalCases = numberOfTestCases (post-snapshot execution units) × numberOfRuns`.
