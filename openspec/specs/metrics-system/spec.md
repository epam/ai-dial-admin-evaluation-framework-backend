...
- **THEN** exactly one write SHALL succeed; the other SHALL fail on the unique index (SQLSTATE 23505) and its whole provider transaction SHALL roll back, so no second version with content identical to the winner's is stored
- **AND** the sync job SHALL treat that failure as an expected outcome (logged, no immediate retry) and the next scheduled run SHALL re-sync that provider
...
- Version uniqueness is a UNIQUE **index** (`V1.30`), not a UNIQUE constraint, and it replaced the non-unique V1.9 index on the same columns: a constraint cannot declare `schema_version DESC`, and only a DESC second column can serve the `DISTINCT ON` ordering above, so a constraint would have left two indexes over the same pair. Nothing FK-references the pair, so an index is sufficient. `PostgresMetricDeclarationVersionRepository.save` computes `MAX(schema_version) + 1` **without** locking the parent `metric_declarations` row, and that index is the only guard. Locking was considered and rejected: the caller's change detection (`MetricProviderSyncService.differsFromLatest`) runs before the insert, so a waiter would unblock, never re-check, and persist a second version whose schemas are identical to the winner's - durable inflation, since TSMDs reference version ids. A `SELECT ... FOR UPDATE` would also conflict with the `FOR KEY SHARE` that FK checks take on `metric_declarations` (`V1.9`, `V1.13`), blocking inserts that never blocked before. Instead `MetricProviderSyncJob` treats 23505 from a provider's transaction as an expected concurrent-sync outcome (info log, no retry; the next scheduled run re-syncs), and `MetricProviderSyncService.syncOne` iterates a provider's metrics sorted by name so concurrent instances take declaration row locks in the same sequence - that removes the ABBA cycle, so the race surfaces as a clean 23505 rather than a 40P01 deadlock.
