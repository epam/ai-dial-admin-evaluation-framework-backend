...
- Tests: `JooqStructuredQueryExecutorExtenderTest` (no extenders, one adding keys, a throwing extender, a throwing extender not suppressing a later one, a null-returning extender, a null-returning extender not suppressing a later one), `OverallScoreTestSuiteRunsPageExtenderTest` (including a multi-row page proving non-overwriting and row-order preservation).
