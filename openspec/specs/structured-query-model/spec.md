...
- Outbound reuse: `experimental.query.model` is a pure-carrier package, not covered by the
  `experimentalService`/`experimentalWeb` ArchUnit layer restriction (verified empirically — a
  `service.domain` class importing it does not trip `LayeredArchitectureTest`), so it may be built and
  serialized directly by code outside `experimental.query.*`. `service.domain.RunCostQueryBuilder`
