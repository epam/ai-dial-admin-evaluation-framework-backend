...
### Requirement: Multi-turn fields exposed on TestCase DTOs
The `multiTurnId` and `turnIndex` fields SHALL be present on `TestCaseRequestDto`, `TestCaseResponseDto`, and `TestCaseBatchPutItemDto`. On response DTOs they SHALL be `null` for single-turn rows. The bulk-patch whitelist SHALL remain `{testCaseName, data}` — `multiTurnId` and `turnIndex` are NOT patchable via bulk PATCH (nor via the single-case merge patch, which already restricts patchable fields to `{testCaseName, data}`).
Status: **Planned**

#### Scenario: Response includes multi-turn fields
- **WHEN** client calls `GET /api/v1/datasets/{datasetId}/test-cases/{testCaseId}` for a multi-turn-turn row
- **THEN** the response SHALL include `multiTurnId` and `turnIndex` with their stored values

#### Scenario: Single-turn response shows null fields
- **WHEN** client fetches a single-turn TestCase
- **THEN** the response SHALL carry `multiTurnId = null` and `turnIndex = null`

#### Scenario: Batch PUT item carries multi-turn fields
- **WHEN** client submits a batch-put item with `multiTurnId` and `turnIndex`
- **THEN** system SHALL persist the item as a multi-turn-turn row subject to the same per-row validation (both-or-neither, UUID form, turn bounds)

#### Scenario: Bulk patch cannot change multi-turn fields
- **WHEN** client submits a bulk PATCH body containing `multiTurnId` or `turnIndex`
- **THEN** system SHALL NOT apply those fields (whitelist is `{testCaseName, data}`); the stored `multiTurnId`/`turnIndex` SHALL remain unchanged

### Requirement: CSV import/export reserved multi-turn columns
CSV import and export SHALL recognize `multiTurnId` and `turnIndex` as reserved column headers using the same reserved-column mechanism as `testCaseName` (the `TEST_CASE_NAME_HEADER` pattern). Reserved columns map to the corresponding top-level TestCase columns and SHALL NOT be placed into the `data` map or contribute to schema auto-detection. All non-reserved columns continue to map into `data`. Import applies the same per-row validation as the API (both-or-neither, valid UUID, turn bounds) and reports violations per row; export emits `multiTurnId` and `turnIndex` columns alongside `testCaseName`.
Status: **Planned**

#### Scenario: Import maps reserved multi-turn columns to top-level columns
- **WHEN** client imports a CSV whose header contains `testCaseName`, `multiTurnId`, `turnIndex`, and data columns
- **THEN** system SHALL populate the top-level `multiTurnId`/`turnIndex` columns from the reserved headers and map only the remaining columns into `data`

#### Scenario: Reserved columns excluded from schema auto-detection
- **WHEN** a CSV with `multiTurnId`/`turnIndex` headers is imported into a dataset with empty schema
- **THEN** the auto-detected `testCaseSchema` SHALL NOT contain fields named `multiTurnId` or `turnIndex`

#### Scenario: Import per-row validation of multi-turn columns
- **WHEN** a CSV row has a `multiTurnId` value but a blank `turnIndex` (or a malformed UUID, or an out-of-range `turnIndex`)
...
#### Scenario: Export includes reserved multi-turn columns
- **WHEN** client exports test cases via `GET /api/v1/datasets/{datasetId}/test-cases/export.csv`
- **THEN** the CSV SHALL include `multiTurnId` and `turnIndex` columns (empty for single-turn rows) in addition to `testCaseName`, and non-reserved data columns follow schema order

### Requirement: Per-turn uniqueness within a multi-turn
In addition to the existing case-insensitive `(dataset_id, LOWER(test_case_name))` uniqueness (which is unchanged — every turn still needs a globally distinct name within its dataset), the service SHALL enforce a defensive per-turn uniqueness on `(dataset_id, multi_turn_id, turn_index)` for grouped rows (i.e. where `multi_turn_id IS NOT NULL`). A create, replace, or import that would produce a duplicate `turnIndex` within the same `(dataset_id, multiTurnId)` SHALL be rejected with HTTP 409.
Status: **Planned**

#### Scenario: Duplicate turn index rejected
- **WHEN** a multi-turn already has a row at `turnIndex = 1` and client creates another row with the same `multiTurnId` and `turnIndex = 1`
- **THEN** system SHALL respond with HTTP 409

#### Scenario: Distinct turn indices in same multi-turn succeed
- **WHEN** client creates rows with the same `multiTurnId` and distinct `turnIndex` values `0`, `1`, `2`
- **THEN** system SHALL create all three rows successfully

#### Scenario: Existing name uniqueness unaffected
- **WHEN** two rows in the same multi-turn are created with distinct `turnIndex` values but the same `testCaseName`
...
- Multi-turn fields: `TestCaseRequestDto` / `TestCaseResponseDto` / `TestCaseBatchPutItemDto` add top-level `multiTurnId`/`turnIndex` (response DTOs surface `null` for single-turn rows); `TestCaseController` accepts/returns them on create, replace, get, and list; bulk-patch payload binding stays limited to `{testCaseName, data}`. `TestCaseValidationService` applies per-row checks (both-or-neither, UUID well-formedness, `0 <= turnIndex < MAX_MULTI_TURN_TURNS`); contiguity/completeness is out of scope here. `CsvImportService` extends the reserved-column set beyond `TEST_CASE_NAME_HEADER` to include `multiTurnId`/`turnIndex` (routed to top-level columns, excluded from `data` and schema auto-detection, validated per row on import/export). Defensive per-turn uniqueness on `(dataset_id, multi_turn_id, turn_index)` for grouped rows.
