...
returning the average per-call price of test-case execution calls and the average per-call price of
metric-evaluation calls for the identified run, sourced from dial-adas usage-log data correlated by the
run's id and execution phase. Status: Implemented.
...
`AdasCostQueryBuilder` builds the outbound query as a real `com.epam.aidial.evaluation.query.model.StructuredQuery` (the same typed AST used for this service's own `POST /api/v1/queries/execute`), rather than hand-rolled `ObjectNode`s — dial-adas's query DSL is the same wire grammar, confirmed against a real deployment, so this is the canonical shape rather than a coincidentally similar one. `DialAdasClient.executeAggregate` accepts a `StructuredQuery` directly; Spring's shared `JsonMapper` bean (`NON_NULL` inclusion) serializes it, dropping the unset `having`/`sort`/`page` fields. The one field dial-adas doesn't show in examples but does accept is `distinct: false` (a required primitive on `StructuredQuery`/`FnExpr`) — harmless, since it's a legitimate field in their own shared schema. See [Query DSL patterns](../../../docs/patterns/README.md) for the underlying model; this is the first reuse of that model as an *outbound* client payload rather than an inbound request parsed by this service.
