...
- Multi-turn is not a suite-level concern: it is emergent from per-turn multi-turn rows (`multiTurnId` + `turnIndex`) in the bound dataset (see `multi-turn` and `test-cases`). `SuiteValidationService.validateDeploymentSuite` runs the normal `inputBindings` cross-validation.
