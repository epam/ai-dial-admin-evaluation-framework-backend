...
- Multi-turn is no longer a suite-level concern: there is no `multiTurn` flag on `data.db.model.TestSuite`, the suite DTOs, or `SuiteSnapshotDto`, and no `test_suites.multi_turn` column. Multi-turn is emergent from per-turn conversation rows (`conversationId` + `turnIndex`) in the bound dataset (see `multi-turn-conversation` and `test-cases`); `SuiteValidationService.validateDeploymentSuite` runs the normal single-turn `inputBindings` cross-validation only (no `validateMultiTurnBody`).
