...
## Purpose
This spec defines the typed jOOQ DSL conventions for the data access layer. All production SQL queries are constructed through type-safe jOOQ DSL references generated from the live database schema. The spec covers the codegen pipeline, schema drift guard, ArchUnit fence, JSONB path accessor isolation, and DSLContext bean wiring.
...
- Analytics jOOQ generator excludes frozen cross-datasource tables by name: `build.gradle` uses `.withExcludes("flyway_schema_history|run_metric_snapshots")`.
...
- The exclude-by-name pattern is a permanent convention, not specific to `run_metric_snapshots`: any future table whose canonical binding moves to the other datasource while a frozen copy remains physically present follows it.
