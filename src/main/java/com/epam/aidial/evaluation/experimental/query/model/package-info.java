 ...
 *
 * <p>Mirrors the §1–§7 wire grammar of {@code docs/experimental/structured-query-model.md}: a
 * single structured JSON query (filter / sort / pagination / projection / aggregation) that
 * clients POST and the server later validates against a per-entity allowlist and translates to
 * parameterized SQL. This package contains <strong>only</strong> the immutable request shape —
 * validation (§9), SQL translation, and the response envelopes (§7.4/§8.2) are intentionally
 * out of scope while the model is exploratory.
 *
