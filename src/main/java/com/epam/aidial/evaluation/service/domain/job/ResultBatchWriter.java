...
public class ResultBatchWriter {
    ...
    public static class RunBuffer {
        ...
        // Progress is counted per conversation (one unit per completed test-case run), not per persisted
    ...
    /**
     * Adds a single-conversation result to the buffer (single-turn case, or a synthetic ERROR row).
    ...
    /**
     * Adds all result rows of one conversation to the buffer (one row for single-turn; N rows for a
     * multi-turn case). Flushes when batch size is reached and advances run progress by exactly <b>one</b>
     * unit regardless of how many rows the conversation wrote. Thread-safe via ReentrantLock.
     */
    public void addResults(RunBuffer buffer, List<TestCaseRunResult> results) {
        ...
        // One progress unit per conversation, regardless of how many rows it wrote. Counted here; the SSE
        // notification is emitted on flush (below) carrying this cumulative conversation count.
    ...
    private void doFlush(RunBuffer buffer, List<TestCaseRunResult> batch) {
        ...
        // Progress is reported in conversation units (one per completed test-case run), not rows — so a
