-- last_turn_index = the maximum authored turn_index among a conversation's surviving turns.
-- Used to evaluate turn position (turn.last) correctly once surviving turns may be non-contiguous
-- (a turn disabled or filtered out at the start/middle/end no longer breaks the conversation, so the
